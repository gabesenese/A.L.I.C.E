...
class ALICE:
    ...
    def __init__(
        ...
        try:
            ...
            if self.runtime_mode_config.enable_proactive_loops:
                ...
                from ai.planning.proactive_intelligence import (
            ...
            if self.runtime_mode_config.enable_autonomous_agent:
                ...
                from ai.planning.autonomous_execution_loop import (
    ...
    def _promote_goal_statement_intent(
        ...
        text = str(user_input or "").lower().strip()
        if "difference between" in text and any(
            t in text for t in ("agentic ai", "generative ai", "agent", "assistant")
        ):
            return (
                "conversation:question",
                dict(entities or {}),
    ...
    def _handle_weather_followup(self, user_input: str, intent: str) -> Optional[str]:
        ...
        def _rain_outlook_reply(time_label: str, weather_line: str) -> Optional[str]:
            asks_rain = bool(
                re.search(
                    r"\b(rain|raining|drizzle|shower|storm|thunder|precip)\b",
                    input_lower,
                )
                and re.search(
                    r"\b(will|gonna|going\s+to|is\s+it|chance|expect|make\s+sure|not)\b",
                    input_lower,
                )
            )
    ...
    def _normalize_weather_intent_for_time_range(
        ...
        if (
            not has_weather_scope
            and not self._is_weather_clothing_time_range_request(text)
            and normalized_intent != "weather:current"
        ):
            return intent, float(intent_confidence or 0.0)
    ...
    def _is_answerability_direct_question(self, user_input: str) -> bool:
        ...
        if "difference between" in text and any(
            k in text for k in ("agentic ai", "generative ai", "agent", "assistant")
        ):
            return True
    ...
    def _answerability_gate_fallback_response(self, user_input: str) -> str:
        ...
        if "existing framework" in text or "framework" in text:
            return "Runtime contract pipeline, executive decision framework, and bounded autonomy framework work together with memory and verification layers to keep actions grounded."
    ...
    def process_input(self, user_input: str, use_voice: bool = False) -> str:
        ...
        """
        try:
            if not hasattr(self, "structured_logger") and callable(
                getattr(self, "_process_input_internal", None)
            ):
                ...
                if getattr(self, "response_formulator", None):
                    user_resp = self.response_formulator.generate(
                        intent=str(
                            getattr(self, "last_intent", "")
                            or getattr(
                                self, "_last_routed_intent", "conversation:general"
                            )
                        ),
                        context={"user_input": user_input, "response": text},
                        tool_results=dict(
                            getattr(self, "_last_plugin_result", {}) or {}
                        ),
            ...
            if is_enabled("contract_pipeline") and getattr(
                self, "contract_pipeline", None
            ):
                try:
                ...
                for pronoun in pronouns:
                    if pronoun in words:
                        ...
                        if resolved:
                            ...
                            if not _skip:
                                self._think(
                                    f"Reference resolution: '{pronoun}' → '{resolved}'"
                                )
                                if "resolved_reference" not in entities:
            ...
            if any(
                ...
                self._think(
                    "Correction/disagreement detected → skipping plugins, using LLM"
                )
            ...
            if self._is_issue_report_input(user_input):
                ...
                if any(intent.startswith(domain) for domain in _tool_domains):
                    self._think(
                        "Issue-report text detected → forcing conversational routing"
                    )
                    intent = "conversation:general"
            ...
            if (
                ...
                self._think(
                    "Low-confidence tool intent without action cues → conversational fallback"
                )
                intent = "conversation:general"
            ...
            if self._is_wake_word_only_input(user_input):
                self._think(
                    "Wake-word-only input → concise conversational acknowledgment"
                )
            ...
            if weather_followup_early:
                ...
                self._think(
                    "Weather follow-up (early) → answering from stored data (no LLM)"
                )
                self._store_interaction(
            ...
            if is_pure_conversation:
                self._think(
                    "Fast path: pure conversation → trying conversational engine"
                )
                ...
                if (
                    ...
                    if self.conversational_engine.can_handle(
                        ...
                        if response:
                            self._think(
                                "Conversational engine → immediate response (learned pattern)"
                            )
                        ...
                        self._think(
                            "Conversational engine → no learned pattern, will use LLM"
                        )
                ...
                if intent == "greeting" and not explicit_greeting_input:
                    self._think(
                        "Intent labeled greeting, but input is not explicit greeting → continue normal routing"
                    )
                    intent = "conversation:general"
            ...
            if hasattr(self, "reasoning_engine") and self.reasoning_engine:
                try:
                    ...
                    if resolution.bindings:
                        self._think(
                            f"Reference resolution → {list(resolution.bindings.keys())} → {[r.resolved_label for r in resolution.bindings.values()]}"
                        )
                    ...
                    if goal_res.cancelled and goal_res.message:
                        ...
                        self._think(
                            f"Goal cancelled → continuing with intent={intent!r}"
                        )
                    if goal_res.revised:
            ...
            _fast_lane_enabled = self._should_use_fast_llm_lane(
                ...
                plugin_scores=getattr(nlp_result, "plugin_scores", {})
                if nlp_result
                else {},
            )
            if _fast_lane_enabled:
                ...
                if _fast_response:
                    ...
                    self._store_interaction(
                        user_input, _fast_response, intent, entities
                    )
            ...
            if (
                ...
                self._think(
                    "Rich conceptual build prompt detected → overriding clarification intent"
                )
                intent = (
            ...
            if (
                ...
                self._think(
                    f"Low confidence ({intent_confidence:.2f}) but active goal → using goal intent: {active_goal.intent}"
                )
                ...
                if active_goal and intent_confidence < 0.7 and _rich_conceptual_prompt:
                    self._think(
                        "Rich conceptual prompt → forcing fresh intent, not reusing goal intent"
                    )
                elif (
                    active_goal
                    and intent_confidence < 0.7
                    ...
                    self._think(
                        "Active goal is clarification_needed → not reusing goal intent"
                    )
                elif (
                    active_goal
                    and intent_confidence < 0.7
                    ...
                    self._think(
                        "Topic shift (low overlap with goal) → not reusing goal intent"
                    )
                elif (
                    active_goal
                    and intent_confidence < 0.7
            ...
            if _cognitive_guidance:
                ...
                if _penalty > 0.0:
                    ...
                    self._think(
                        f"Immediate feedback penalty applied ({_penalty:.2f}) → confidence={intent_confidence:.2f}"
                    )
            ...
            if _pre_route_guard.get("block"):
                ...
                self._think(
                    f"Pre-route plausibility guard → {_pre_route_guard.get('reason', 'unknown')}"
                )
            ...
            if _executive_decision.action in ("use_llm", "answer_direct"):
                ...
                if _self_answer.get("block_llm"):
                    ...
                    if response:
                        self._think(
                            f"Self-answer-first gate → {_self_answer.get('reason', 'native')}"
                        )
                        self._cache_put(user_input, intent, response)
                        self._store_interaction(user_input, response, intent, entities)
                        if use_voice and self.speech:
            ...
            if _executive_decision.action == "ask_clarification":
                if self._should_answer_first_without_clarification(
                    ...
                    self._think(
                        "Executive clarification overridden → answer-first conversational path"
                    )
            ...
            if _execution_mode == "conversational_intelligence":
                ...
                _turn_policy = (
                    getattr(self, "turn_routing_policy", None)
                    or get_turn_routing_policy()
                )
                _routing_decision = _turn_policy.decide(
                    executive_action=_executive_decision.action,
                    runtime_allow_tools=bool(
                        _runtime_controls.get("allow_tools", True)
                    ),
                    runtime_preference=str(
                        _runtime_controls.get("routing_preference") or "balanced"
                    ),
            ...
            if (
                getattr(self, "roadmap_stack", None)
                and _execution_mode != "conversational_intelligence"
            ):
            ...
            if not _should_try_plugins:
                if _execution_mode == "conversational_intelligence":
                    ...
                    self._think(
                        "Short follow-up (no command words, no goal) → skipping plugins, using LLM"
                    )
            else:
                self._think(
            ...
            if _should_try_plugins:
                ...
                if not plugin_result and intent_confidence < 0.6:
                    self._think(
                        "Low confidence + no plugin match → using LLM with context"
                    )
            ...
            if plugin_result:
                ...
                if not _tool_verified:
                    self._think(
                        "Tool output not verified → clarification before state updates"
                    )
                    response = (
                ...
                if plugin_result.get("formulate", False):
                    self._think(
                        f"Plugin requested formulation → Alice will learn to respond"
                    )
                ...
                if self.foundations and self.foundation_mode in [
                    ...
                    try:
                        self._think(
                            f"Foundation systems active (mode={self.foundation_mode}) → generating context-aware response"
                        )
                    ...
                    try:
                        self._think(
                            "Foundation parallel mode → running both systems for comparison"
                        )
                        foundation_result = self.foundations.process_interaction(
                            user_id=self.user_name,
                            user_input=user_input,
                ...
                if not response:
                    self._think(
                        "Response generation failed → Alice uses direct phrase fallback (no Ollama)"
                    )
                ...
                if hasattr(self, "reasoning_engine") and self.reasoning_engine:
                    ...
                    self._think(
                        f"Verifier → verified={verification.verified} goal_fulfilled={verification.goal_fulfilled}"
                    )
                    ...
                    if (
                        ...
                        if message_code == "weather:forecast":
                            ...
                            if _mentioned_tr:
                                ...
                                if _tr_response:
                                    self._think(
                                        f"Filtered forecast for '{_mentioned_tr}' — replacing full 7-day table"
                                    )
            ...
            if self.phrasing_learner:
                ...
                if self.phrasing_learner.can_phrase_myself(
                    ...
                    if _learned_response:
                        self._think(
                            f"Alice answered from learned patterns — Ollama not needed for '{intent}'"
                        )
                        if getattr(self, "response_optimizer", None):
            ...
            try:
                ...
                if _learn_decision != "store":
                    self._think(
                        f"Executive learning authority → {_learn_decision} (skip long-term learning)"
                    )
                else:
            ...
            if hasattr(self, "usage_analytics"):
                try:
                    ...
                    llm_used = (
                        "ollama_phrased_response" in locals()
            ...
            duration = time.time() - start_time
            route_taken = (
                "plugin" if "plugin_result" in locals() and plugin_result else "llm"
            )
...
Generate only the farewell (1 sentence), no other text. Be warm and friendly."""
    ...
    def shutdown(self):
        ...
        if getattr(self, "continuous_learning", None):
