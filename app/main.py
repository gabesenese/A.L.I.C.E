...
class ALICE:
    ...
    def _process_input_legacy(self, user_input: str, use_voice: bool = False) -> str:
        """Legacy fallback — redirects to the contract pipeline.

        The contract pipeline is the only supported path. This method is
        reached only when pipeline.run_turn raises inside
        turn_orchestrator.run_default_turn. All memory, learning, and
        personality updates happen inside the pipeline via companion_runtime.
        """
        pipeline = getattr(self, "contract_pipeline", None)
        if pipeline is not None:
            result = pipeline.run_turn_sync(
                user_input=user_input,
                user_id=str(getattr(self, "user_name", "") or "User"),
                turn_number=int(getattr(self, "_turn_count", 0) or 0),
            )
            if result and result.handled and result.response_text:
