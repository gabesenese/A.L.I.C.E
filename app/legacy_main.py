...
class ALICE:
    ...
    def _alice_direct_phrase(self, response_type: str, alice_response: Dict[str, Any]) -> Optional[str]:
            ...
            if isinstance(summary, dict):
                ...
                if summary.get('key_points'):
                    ...
                    for pt in summary['key_points'][:5]:
                        lines.append(f"- {pt}")
                return "\n".join(lines) if len(lines) > 2 else f"Summary of **{title}** ΓÇö no structured data available."
        ...
        if response_type == 'weather_forecast':
            ...
            def _fmt_table_row(d, day: dict) -> str:
                ...
                temp  = f"{_format_temp(low)}┬░ to {_format_temp(high)}┬░C" if (high is not None and low is not None) else "ΓÇö"
        ...
        try:
            ...
            if llm_response.success and llm_response.response:
                ...
                if self.phrasing_learner.can_phrase_myself(_thought_for_learning, tone):
                    self._think(f"Alice learned '{response_type}' ΓÇö can now phrase independently!")
                ...
                logger.warning("[ALICE] Ollama phrasing failed ΓÇö using Alice's direct fallback")
                return self._clamp_final_response(
                    content_str,
                    tone=tone,
                    response_type=str(response_type or ''),
    ...
    def _promote_learning_goal_intent(
        ...
        self._think(f"Reasoning goal detected ΓåÆ planning study flow for: {entities.get('topic', 'topic')}")
            ...
            try:
                ...
                if result:
                    ...
                    if rain_reply:
                        ...
                        logger.info(f"Weather follow-up (rain outlook) ΓåÆ {rain_reply[:60]}...")
                        return rain_reply
                    self._think(f"Forecast formatter returned: {result[:60]}...")
                    logger.info(f"Weather follow-up (stored, time-range) ΓåÆ {result[:60]}...")
                    return result
    ...
    def _process_input_internal(self, user_input: str, use_voice: bool = False) -> str:
        ...
        try:
                ...
                for pronoun in pronouns:
                    if pronoun in words:
                        ...
                        if resolved:
                            ...
                            if not _skip:
                                self._think(f"Reference resolution: '{pronoun}' ΓåÆ '{resolved}'")
                                if 'resolved_reference' not in entities:
            ...
            if intent == "reminder:cancel":
                ...
                if proactive:
                    ...
                    if proactive.cancel_reminder(keyword):
                        return f"Done ΓÇö I've cancelled the reminder about \"{keyword}\"."
                    else:
            ...
            if any(_input_for_correction == m or _input_for_correction.startswith(m)
                ...
                self._think("Correction/disagreement detected ΓåÆ skipping plugins, using LLM")
            ...
            if self._is_issue_report_input(user_input):
                ...
                if any(intent.startswith(domain) for domain in _tool_domains):
                    self._think("Issue-report text detected ΓåÆ forcing conversational routing")
                    intent = "conversation:general"
            ...
            if (
                ...
                self._think("Low-confidence tool intent without action cues ΓåÆ conversational fallback")
                intent = "conversation:general"
            ...
            if self._is_wake_word_only_input(user_input):
                self._think("Wake-word-only input ΓåÆ concise conversational acknowledgment")
            ...
            if weather_followup_early:
                ...
                self._think("Weather follow-up (early) ΓåÆ answering from stored data (no LLM)")
            ...
            if is_pure_conversation:
                self._think("Fast path: pure conversation ΓåÆ trying conversational engine")
                ...
                if hasattr(self, 'conversational_engine') and self.conversational_engine:
                    ...
                    if self.conversational_engine.can_handle(user_input, intent, context):
                        ...
                        if response:
                            self._think("Conversational engine ΓåÆ immediate response (learned pattern)")
                        ...
                        self._think("Conversational engine ΓåÆ no learned pattern, using native greeting/fallback path")
                ...
                if intent == "greeting" and not explicit_greeting_input and not high_confidence_greeting_lock:
                    self._think("Intent labeled greeting, but input is not explicit greeting ΓåÆ continue normal routing")
                    intent = "conversation:general"
                ...
                if intent == "greeting" and (explicit_greeting_input or high_confidence_greeting_lock):
                    ...
                    if hasattr(self, 'conversational_engine') and self.conversational_engine:
                        ...
                        if self.conversational_engine.can_handle(user_input, intent, conv_context):
                            ...
                            if response:
                                self._think("Greeting ΓåÆ using learned pattern (no LLM)")
                                self._cache_put(user_input, intent, response)
                                self._store_interaction(user_input, response, intent, entities)
                                if use_voice and self.speech:
                                    self.speech.speak(response, blocking=False)
            ...
            if hasattr(self, 'reasoning_engine') and self.reasoning_engine:
                try:
                    ...
                    if resolution.bindings:
                        self._think(f"Reference resolution ΓåÆ {list(resolution.bindings.keys())} ΓåÆ {[r.resolved_label for r in resolution.bindings.values()]}")
                    ...
                    if goal_res.cancelled and goal_res.message:
                        ...
                        self._think(f"Goal cancelled ΓåÆ continuing with intent={intent!r}")
                    if goal_res.revised:
            ...
            if _rich_conceptual_prompt and intent == "conversation:clarification_needed":
                self._think("Rich conceptual build prompt detected ΓåÆ overriding clarification intent")
            ...
            if (
                ...
                self._think(f"Low confidence ({intent_confidence:.2f}) but active goal ΓåÆ using goal intent: {active_goal.intent}")
            ...
            if _cognitive_guidance:
                ...
                if _penalty > 0.0:
                    ...
                    self._think(f"Immediate feedback penalty applied ({_penalty:.2f}) ΓåÆ confidence={intent_confidence:.2f}")
            ...
            if _pre_route_guard.get("block"):
                ...
                self._think(f"Pre-route plausibility guard ΓåÆ {_pre_route_guard.get('reason', 'unknown')}")
                ...
                if self._self_answer_first_can_override(_executive_decision.action):
                    ...
                    if _self_answer.get("block_llm"):
                        ...
                        if response:
                            ...
                            self._think(f"Self-answer-first gate ΓåÆ {_self_answer.get('reason', 'native')}")
                            self._cache_put(user_input, intent, response)
                            self._store_interaction(user_input, response, intent, entities)
                            if use_voice and self.speech:
                                self.speech.speak(response, blocking=False)
                    ...
                    self._think("Self-answer-first gate bypassed ΓåÆ llm_authority_lock")
            ...
            if _executive_decision.action == "ask_clarification":
                if self._should_answer_first_without_clarification(
                    ...
                    self._think("Executive clarification overridden ΓåÆ answer-first conversational path")
            ...
            if not _should_try_plugins:
                if is_pure_conversation:
                    ...
                    self._think("Short follow-up (no command words, no goal) ΓåÆ skipping plugins, using LLM")
            else:
            ...
            if _should_try_plugins:
                ...
                if not plugin_result and intent_confidence < 0.6:
                    self._think("Low confidence + no plugin match ΓåÆ using LLM with context")
            ...
            if plugin_result:
                ...
                if not _tool_verified:
                    self._think("Tool output not verified ΓåÆ clarification before state updates")
                    response = (
                ...
                if plugin_result.get('formulate', False):
                    self._think(f"Plugin requested formulation ΓåÆ Alice will learn to respond")
                ...
                if self.foundations and self.foundation_mode in ["primary", "exclusive"]:
                    try:
                        self._think(f"Foundation systems active (mode={self.foundation_mode}) ΓåÆ generating context-aware response")
                    ...
                    try:
                        self._think("Foundation parallel mode ΓåÆ running both systems for comparison")
                        foundation_result = self.foundations.process_interaction(
                            user_id=runtime_user_name,
                            user_input=user_input,
                            intent=intent,
                ...
                if not response:
                    self._think("Response generation failed ΓåÆ Alice uses direct phrase fallback (no Ollama)")
                    ...
                    if not response:
                        ...
                        if success:
                            ...
                            response = f"Done." if not subject else f"Done ΓÇö {subject}."
                        else:
                ...
                if hasattr(self, 'reasoning_engine') and self.reasoning_engine:
                    ...
                    self._think(f"Verifier ΓåÆ verified={verification.verified} goal_fulfilled={verification.goal_fulfilled}")
                    ...
                    if plugin_name == 'WeatherPlugin' and success and plugin_result.get('data'):
                        ...
                        if message_code == 'weather:forecast':
                            ...
                            if _mentioned_tr:
                                ...
                                if _tr_response:
                                    self._think(f"Filtered forecast for '{_mentioned_tr}' ΓÇö replacing full 7-day table")
            ...
            if self.phrasing_learner:
                ...
                if self.phrasing_learner.can_phrase_myself(_conv_thought, _tone_for_check):
                    ...
                    if _learned_response:
                        self._think(f"Alice answered from learned patterns ΓÇö Ollama not needed for '{intent}'")
                        if getattr(self, 'response_optimizer', None):
            ...
            try:
                ...
                if _learn_decision != "store":
                    self._think(f"Executive learning authority ΓåÆ {_learn_decision} (skip long-term learning)")
                else:
                ...
                try:
                    ...
                    if stats['relationship_types']:
                        ...
                        for rel_type, count in stats['relationship_types'].items():
                            print(f"   ΓÇó {rel_type.replace('_', ' ').title()}: {count}")
                    ...
                    if stats['recent_relationships']:
                        ...
                        for rel in stats['recent_relationships'][:5]:
                            ...
                            print(f"   ΓÇó {source} {rel_type} {target} (confidence: {confidence:.2f})")
            ...
            if match:
                ...
                for rel_type, connections in by_type.items():
                    if len(connections) == 1:
                        ...
                        response_parts.append(f"ΓÇó {rel_type.replace('_', ' ')}: {', '.join([c.replace(rel_type.replace('_', ' '), '').strip() for c in connections])}")
                ...
                if second_entity:
                    ...
                    if specific_rels:
                        ...
                        for rel in specific_rels:
                            if rel.source_entity == entity_name.lower():
                                response_parts.append(f"ΓÇó {entity_name.title()} {rel.relationship_type.replace('_', ' ')} {rel.target_entity.title()}")
                            else:
                                response_parts.append(f"ΓÇó {rel.source_entity.title()} {rel.relationship_type.replace('_', ' ')} {entity_name.title()}")
