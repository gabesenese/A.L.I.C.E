...
class ALICE:
    ...
    def __init__(
        ...
        self.structured_logger.info(
            "Foundation systems will initialize after LLM engine",
        ...
        try:
            ...
            self.structured_logger.set_context(
                user=self.user_name, session_id=f"{time.time()}"
            )
            ...
            logger.info(
                "Semantic classifier lazy-init enabled (startup pre-warm skipped)."
            )
            ...
            # Area 1ΓÇô8: advanced intelligence components
            try:
            ...
            logger.info(
                f"[OK] Knowledge engine ready - Alice has learned {stats['total_entities']} entities, {stats['total_relationships']} relationships"
            )
            ...
            logger.info(
                f"[OK] User profile loaded - {profile_summary.get('interactions', 0)} interactions, learning preferences"
            )
            ...
            try:
                ...
                logger.info(
                    "[OK] Foundation systems ready - Response Variance, Personality Evolution, Context Graph active"
                )
            except Exception as e:
            ...
            logger.info("≡ƒÜÇ Initializing 10 Tier Improvements (quarantine-aware)...")
            try:
                # Tier 1: High-Impact Wins
                logger.info("  Γö£ΓöÇ Tier 1: Initializing long-session coherence...")
                ...
                logger.info("  Γö£ΓöÇ Tier 1: Initializing capability constraints...")
                ...
                logger.info("  Γö£ΓöÇ Tier 1: Initializing result quality scorer...")
                ...
                logger.info("  Γö£ΓöÇ Tier 1: Initializing goal alignment tracker...")
                ...
                logger.info("  Γö£ΓöÇ Tier 2: Initializing tone trajectory engine...")
                ...
                logger.info("  Γö£ΓöÇ Tier 2: Initializing pattern-based nudger...")
                ...
                logger.info("  Γö£ΓöÇ Tier 3: Initializing system state API...")
                ...
                logger.info("  Γö£ΓöÇ Tier 3: Initializing weak-spot detector...")
                ...
                logger.info("  Γö£ΓöÇ Tier 4: Initializing multi-goal arbitrator...")
                ...
                logger.info("  ΓööΓöÇ Tier 4: Initializing routing decision logger...")
            ...
            if getattr(self, "nlp", None) and hasattr(self.nlp, "attach_llm_gateway"):
                try:
                    ...
                    logger.debug(
                        f"[NLP] Failed to attach LLM gateway for intent fallback: {e}"
                    )
            ...
            self.response_formulator = get_response_formulator(
                phrasing_learner=self.phrasing_learner, llm_gateway=self.llm_gateway
            )
            stats = self.response_formulator.get_stats()
            logger.info(
                f"[OK] Response formulator ready - Alice can independently formulate {stats['independent_actions']}/{stats['total_templates']} action types"
            )
    ...
    def _init_companion_daemon(self) -> None:
        ...
        try:
            ...
            start_paused = os.getenv("ALICE_COMPANION_PAUSED", "0").strip().lower() in {
                "1",
                "true",
                "yes",
                "on",
            ...
            def notify(message: str, priority: str = "normal") -> None:
                priority_name = str(
                    getattr(priority, "name", priority) or "normal"
                ).upper()
                event_priority = getattr(
                    EventPriority, priority_name, EventPriority.NORMAL
                )
    ...
    def _select_tone(self, intent: str, context: Any, user_input: str) -> str:
        ...
        """
        # ΓöÇΓöÇ Interaction policy overrides (mood-driven) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        # ΓöÇΓöÇ Response knob bandit override ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        # Consult the learned style policy.  High empathy ΓåÆ calm tone; high
        # directness ΓåÆ precise tone.  Only overrides when the bandit has enough
    ...
    def _formulate_from_plugin_data(
        ...
        - Alice thinks: "Very cold ΓåÆ yes, definitely wear layers"
        ...
        # Detect weather by CONTENT as well as intent ΓÇö NLP sometimes misfires
        ...
        if _is_weather_data:
            ...
            # also a weather response ΓÇö not whenever weather appeared anywhere.
            ...
            # Forecast data (no current temperature) ΓÇö Alice formats directly
            if forecast and isinstance(forecast, list) and len(forecast) > 0:
                return {
        ...
        # Note/file operations ΓÇö detect by intent OR by the action the plugin returned
    ...
    def _fallback_clarification_response(
        ...
        *,
        ...
        if strict:
            response_seed = (
                "I need one missing detail so I can route this request correctly."
            )
        else:
    ...
    def _rewrite_anthropomorphic_routine_claims(cls, text: str) -> str:
        ...
        for part in re.split(r"(?<=[.!?])\s+|\n+", raw):
            ...
            if cls._contains_anthropomorphic_routine_claim(sentence):
                sentence = re.sub(
                    r"\bwhen\s+i\s+unwind,?\s*", "", sentence, flags=re.IGNORECASE
                )
                sentence = re.sub(
    ...
    def _clamp_final_response(
        ...
        *,
        ...
        if any(pattern in lower for pattern in leak_patterns) or bool(
            internal_contract_regex.search(text)
        ):
        ...
        ) and self.response_formulator.is_internal(text):
            if response_type == "clarification_prompt":
                return self._fallback_clarification_response(
                    user_input,
                    intent=inferred_intent,
    ...
    def _alice_direct_phrase(
        ...
        Ollama is NEVER in control here ΓÇö it is only a teacher when Alice lacks
        phrasing experience for a novel conversational pattern.
        """
        # ΓöÇΓöÇ Notes ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        if response_type == "note_content":
            ...
            # Don't repeat the title as body ΓÇö if a note's content was never
        ...
        if response_type == "note_summary":
            ...
            if isinstance(summary, dict):
                ...
                return (
                    "\n".join(lines)
                    if len(lines) > 2
                    else f"Summary of **{title}** ΓÇö no structured data available."
                )
        ...
        # ΓöÇΓöÇ Weather ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        def _format_temp(value):
            """Format temperature with proper minus sign (ΓêÆ instead of -)"""
            if value is None:
                return None
            # Convert to numeric value
            if isinstance(value, str):
                # Replace any Unicode minus with hyphen for parsing
                value = value.replace("ΓêÆ", "-")
                try:
                    value = float(value)
        ...
        if response_type == "weather_report":
            ...
            if is_followup:
                return (
                    f"Still **{cond_cap}**{loc_str} {temp_str}."
                    if temp is not None
        ...
        if response_type == "weather_advice":
            ...
            if scope_label:
                ...
                if item == "umbrella":
                    umbrella_reason = (
                        "rain is in the forecast" if bool(rainy) else forecast_reason
                    )
                    if force_yes_no:
                        if bool(rainy):
            ...
            if force_yes_no:
                ...
                return (
                    f"No, you likely do not need {item_text} because {reason}{place}."
                )
        ...
        if response_type == "weather_forecast":
            ...
            # ΓöÇΓöÇ Specific weekday ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            ...
            # ΓöÇΓöÇ Weekend: two-row table ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            if wants_weekend:
                weekend_days = []
                for day in forecast:
                    try:
            ...
            # ΓöÇΓöÇ Tomorrow: single bold line ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            if wants_tomorrow:
            ...
            # ΓöÇΓöÇ Full 7-day table ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            lines = [
        ...
        # ΓöÇΓöÇ Capability ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        # ΓöÇΓöÇ Self-analysis / code ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        if response_type == "self_analysis":
            ...
            for f in analyzed[:5]:
                lines.append(f"  ΓÇó {f['path']}: {f.get('lines', 0)} lines")
            for p in points[:5]:
                lines.append(f"  ΓÇó {p}")
            return "\n".join(lines)
        ...
        # Open-ended types ΓÇö return None to let Ollama assist Alice
        return None

    def _generate_natural_response(
        self, alice_response: Dict[str, Any], tone: str, context: Any, user_input: str
    ) -> str:
        """
        Generate natural language response.  Alice is always in control.

        Priority order:
        1. Alice's learned phrasing patterns  (fastest ΓÇö zero LLM)
        ...
        # ΓöÇΓöÇ Step 1: Alice phrases it directly from structured data ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        # ΓöÇΓöÇ Step 2: Learned pattern for open-ended types (Alice independent) ΓöÇ
        ...
        # ΓöÇΓöÇ Step 3: Open-ended ΓÇö Alice asks Ollama for phrasing help ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        # Alice phrasing it naturally ΓÇö she provides the facts, Ollama provides fluency.
        if response_type == "knowledge_answer":
            ...
            if rainy is False:
                content_str = (
                    ...
                    f"User asked: '{user_q}'. No rain is expected ΓÇö advise on whether an umbrella is needed."
                )
            else:
                content_str = (
                    f"Current weather: {temp_str}, {condition}. "
        ...
        try:
            ...
            if llm_response.success and llm_response.response:
                ...
                if self.phrasing_learner.can_phrase_myself(_thought_for_learning, tone):
                    self._think(
                        f"Alice learned '{response_type}' ΓÇö can now phrase independently!"
                    )
                ...
                logger.warning(
                    "[ALICE] Ollama phrasing failed ΓÇö using Alice's direct fallback"
                )
                return self._clamp_final_response(
                    content_str,
                    tone=tone,
    ...
    def _agentic_loop_perceive(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        entities = (
            data.get("entities") if isinstance(data.get("entities"), dict) else {}
        )
    ...
    def _agentic_loop_goal(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if explicit_goal:
            ...
            state_goal = str(
                ...
                or (getattr(self, "_internal_reasoning_state", {}) or {}).get(
                    "conversation_goal"
                )
    ...
    def _agentic_loop_decide(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if phase == "pre_route":
            ...
            force_plugins_for_notes = bool(
                perceived.get("force_plugins_for_notes", False)
            )
    ...
    def _agentic_primary_authority_decision(
        ...
        *,
        ...
        default_action = (
            "use_plugin"
            if str(execution_mode or "").strip() == "operator"
            else "use_llm"
        )
    ...
    def _collect_agentic_readiness_metrics(self) -> Dict[str, float]:
        ...
        if hasattr(self, "metrics") and self.metrics:
            try:
                ...
                clarification_prompts = sum(
                    ...
                    and str(k).startswith("clarification_")
                )
        ...
        if hasattr(self, "realtime_logger") and self.realtime_logger:
            try:
                ...
                for err in recent_errors:
                    ...
                    route = str(
                        ((err or {}).get("context") or {}).get("route") or ""
                    ).lower()
                    if any(
                        token in err_type
                        for token in ("wrong_tool", "route_mismatch", "wrong_domain")
                    ):
                        wrong_tool_events += 1
                    elif err_type == "execution_incomplete" and route in {
                        "tools",
                        "plugin",
                    }:
                        wrong_tool_events += 1
        ...
        if hasattr(self, "execution_journal") and self.execution_journal:
            try:
                ...
                if total > 0:
                    long_horizon_completion_rate = min(
                        1.0, max(0.0, goal_satisfied / float(total))
                    )
        ...
        if hasattr(self, "user_profile") and self.user_profile:
            try:
                ...
                high_conf = int(
                    profile_summary.get("high_confidence_preferences", 0) or 0
                )
                if learned > 0:
                    personalization_satisfaction = min(
                        1.0, max(0.0, high_conf / float(learned))
                    )
        ...
        clarification_precision = 1.0 - (
            float(clarification_prompts) / float(denominator)
        )
        clarification_precision = min(1.0, max(0.0, clarification_precision))
        wrong_tool_rate = min(
            1.0, max(0.0, float(wrong_tool_events) / float(denominator))
        )
        ...
        return {
            ...
            "recovery_success_rate": round(
                min(1.0, max(0.0, recovery_success_rate)), 4
            ),
            "stale_state_rate": round(min(1.0, max(0.0, stale_state_rate)), 4),
            "long_horizon_completion_rate": round(
                min(1.0, max(0.0, long_horizon_completion_rate)), 4
            ),
            "personalization_satisfaction": round(
                min(1.0, max(0.0, personalization_satisfaction)), 4
            ),
        }
    ...
    def _update_agentic_readiness_snapshot(self, force: bool = False) -> Dict[str, Any]:
        ...
        if not force and (
            now
            - float(getattr(self, "_last_agentic_readiness_snapshot_ts", 0.0) or 0.0)
        ) < float(
            getattr(self, "_agentic_readiness_snapshot_interval_seconds", 900.0)
            or 900.0
        ):
    ...
    def _fallback_from_intent(
        ...
        if text:
            ...
            if self._is_project_ideation_turn(
                text, intent
            ) or self._is_project_ideation_request(text):
    ...
    def _promote_learning_goal_intent(
        ...
        self._think(
            f"Reasoning goal detected ΓåÆ planning study flow for: {entities.get('topic', 'topic')}"
        )
    ...
    def _build_turn_execution_outcome(
        ...
        *,
        ...
        verification_passed = bool(
            verification.get("accepted", False) or report.get("accepted", False)
        )
    ...
    def _is_weather_context_relevant_query(
    ...
    def _build_llm_context(
        ...
        if cached:
            self._think("Context cache hit ΓåÆ reusing context")
            return cached
        ...
        if (
            ...
            self._think(f"Goal context ΓåÆ {goal.description[:50]}...")
        ...
        # 8. Notes context injection (Feature #2) ΓÇö surface relevant note snippets
    ...
    def _publish_with_fast_llm_style(
        ...
        *,
        ...
        tone = (
            self._select_tone(intent, None, user_input)
            if hasattr(self, "_select_tone")
            else "helpful"
        )
        phrase_context = {
    ...
    def _is_project_ideation_request(self, user_input: str) -> bool:
        ...
        short_imperative = bool(
            re.match(
                r"^(?:create|build|make|start)\s+(?:an?\s+)?(?:agentic\s+)?(?:ai|assistant|agent|system|project)\b",
                text,
            )
        )
        ...
        has_agentic_ai_phrase = "agentic ai" in text or (
            "agentic" in text and "ai" in text
        )
        return bool(
            has_project_noun
            or domain
            or has_agentic_ai_phrase
            or (short_imperative and len(tokens) <= 8)
        )
    ...
    def _project_ideation_narrowing_question(self, user_input: str) -> str:
        ...
        inferred_domain = (
            domain or self._infer_learning_domain(user_input) or "architecture"
        )
    ...
    def _project_ideation_guidance_response(self, user_input: str) -> str:
        ...
        if self._is_technical_agent_build_request(
            user_input, intent="learning:project_ideation"
        ...
        anchor_line = "A strong place to start is to focus first on memory, tool-use, or conversational quality."
    ...
    def _goal_statement_alignment_response(
        ...
        if marker_set.intersection({"architecture", "orchestration", "vision"}):
            _add_rec(
                "Strengthen orchestration boundaries so routing, execution, and verification are explicitly separated."
            )
    ...
    def _promote_goal_statement_intent(
        ...
        if (
            ...
            self._think(
                "Direct informational question detected -> conversation:question"
            )
            return (
                "conversation:question",
                dict(entities or {}),
                max(float(intent_confidence or 0.0), 0.84),
            )
    ...
    def _is_technical_agent_build_request(
        ...
        *,
        ...
        domain = (
            str(ents.get("project_domain") or ents.get("domain") or "").lower().strip()
        )
    ...
    def _looks_generic_project_management_response(self, response: str) -> bool:
        ...
        listy_pm = bool(
            re.search(r"\b(?:1\.|2\.|3\.)\s*(?:define|research|concept|plan)\b", low)
        )
        return bool((pm_hits >= 2 and tech_hits <= 1) or (listy_pm and tech_hits == 0))

    def _is_agent_transformation_friction_report(
        self, user_input: str, intent: str = ""
    ) -> bool:
        ...
        conversational = str(intent or "").startswith("conversation:") or str(
            intent or ""
        ).startswith("learning:")
    ...
    def _is_practical_agentic_framework_request(self, user_input: str) -> bool:
        ...
        short_framework_prompt = (
            has_framework_cue and has_agentic_scope and len(tokens) <= 7
        )

        return bool(
            has_framework_cue
            and has_agentic_scope
            and (has_build_or_research_cue or short_framework_prompt)
        )
    ...
    def _is_existing_framework_inventory_request(self, user_input: str) -> bool:
        ...
        has_explain = any(
            cue in text
            for cue in ("explain", "walk me through", "break down", "describe")
        )
        has_scope_hint = any(
            cue in text
            for cue in (
    ...
    def _practical_agentic_frameworks_answer(
        ...
        *,
        ...
        wants_multi_agent = any(
            cue in text
            for cue in ("multi-agent", "multi agent", "crew", "coordination")
        )
        wants_production = any(
            cue in text for cue in ("production", "deploy", "reliable", "robust")
        )
        wants_research = any(
            cue in text for cue in ("research", "paper", "compare", "benchmark")
        )
    ...
    def _structured_teaching_mode_response(
        ...
        if not project:
            project = [
                "apply one concept in your current project",
    ...
    def _deterministic_fallback_once(
        self, user_input: str, intent: str
    ) -> Optional[str]:
        ...
        if policy is None:
            ...
            if result and isinstance(
                getattr(self, "_internal_reasoning_state", None), dict
            ):
        ...
        try:
            result = policy.deterministic_once(
                user_input=user_input,
                intent=intent,
                builder=lambda: self._deterministic_knowledge_fallback(
                    user_input, intent
                ),
            )
        except Exception:
    ...
    def _deterministic_knowledge_fallback(
        ...
        if method_topics:
            method_lead = (
                ...
                if any(
                    k in text
                    for k in ("algorithm", "algorithms", "method", "technique")
                )
                else "add"
            )
            segments.append(
        ...
        if project_topics:
            segments.append(
                f"Project priorities include {self._format_compact_list(project_topics)}."
            )
        ...
        if terms:
            ...
            if branch_pick:
                segments.append(
                    f"We can go deeper on {self._format_compact_list(branch_pick)} first."
                )
            elif tail_terms:
                segments.append(
                    f"We can go deeper on {self._format_compact_list(tail_terms)} first."
                )
            else:
        ...
        segments.append(
            "Tell me which branch you want first and I will map the next step."
        )
    ...
    def _dynamic_short_greeting(
        ...
        *,
        ...
        previous = (
            str(getattr(self, "_last_dynamic_greeting", "") or "").strip().lower()
        )
    ...
    def _is_explicit_greeting_input(self, user_input: str) -> bool:
        ...
        return all(
            token in greeting_words or _is_likely_name_token(token) for token in tokens
        )
                    ...
                    if hasattr(self.conversational_engine, "_pick_non_repeating"):
                        ...
                        if picked:
                            ...
                            if (
                                ...
                                and candidate.lower()
                                != str(
                                    getattr(self, "_last_dynamic_greeting", "") or ""
                                )
                                .strip()
                                .lower()
                            ):
        ...
        if self.phrasing_learner:
            ...
            try:
                if self.phrasing_learner.can_phrase_myself(
                    ...
                    phrased = self.phrasing_learner.phrase_myself(
                        greeting_thought, "friendly"
                    )
                    if phrased:
                        ...
                        if (
                            candidate
                            and candidate.lower()
                            != str(getattr(self, "_last_dynamic_greeting", "") or "")
                            .strip()
                            .lower()
                        ):
                            self._last_dynamic_greeting = candidate
    ...
    def _should_reuse_goal_intent(self, user_input: str, goal_description: str) -> bool:
        ...
        if any(w in input_lower for w in ["add", "put", "append", "include"]) and any(
    ...
    def _handle_code_request(
        ...
        if (
            (not has_py_file)
            and self.code_context.get("last_action") == "list"
            and self.code_context.get("last_files_shown")
        ):
        ...
        if not has_py_file and any(
            ...
            for phrase in [
                ...
                "what files",
                "list your code",
                "list your internal code",
                "list internal code",
                "show your internal code",
            ]
        ):
        ...
        if has_py_file:
            ...
            if any(
                ...
                if code_file:
                    ...
                    if (
                        "summarize" in input_lower
                        or "summary" in input_lower
                        or "tell me about" in input_lower
                        or "what does" in input_lower
                        or "describe" in input_lower
                    ):
    ...
    def _handle_weather_followup(self, user_input: str, intent: str) -> Optional[str]:
        ...
        def _rain_outlook_reply(time_label: str, weather_line: str) -> Optional[str]:
            mentions_rain_topic = bool(
                re.search(
                    r"\b(rain|raining|drizzle|shower|storm|thunder|precip|wet)\b",
                    input_lower,
                )
            )
            ...
            if has_schedule_context:
                if appointment_time:
                    appointment_note = f" For your {appointment_time.group(1)} appointment, check the hourly forecast closer to that time."
                else:
            ...
            try:
                ...
                if result:
                    ...
                    if rain_reply:
                        ...
                        logger.info(
                            f"Weather follow-up (rain outlook) ΓåÆ {rain_reply[:60]}..."
                        )
                        return rain_reply
                    self._think(f"Forecast formatter returned: {result[:60]}...")
                    logger.info(
                        f"Weather follow-up (stored, time-range) ΓåÆ {result[:60]}..."
                    )
                    return result
                self._think(
                ...
                logger.error(
                    f"Error formatting forecast for time-range '{mentioned_time_range}': {e}",
                    exc_info=True,
                )
        ...
        if mentioned_day:
            ...
            if bool(forecast_data.get("is_stale")):
                ...
                return None
            self._think(
                f"Using stored forecast data for {mentioned_day} query: {type(forecast_data)} with {len(forecast_data.get('forecast', []))} days"
            )
            ...
            try:
                ...
                if result:
                    ...
                    if rain_reply:
                        ...
                        return rain_reply
                    self._think(f"Forecast formatter returned: {result[:60]}...")
                    logger.info(f"Weather follow-up (stored) ΓåÆ {result[:60]}...")
                    return result
                else:
        ...
        # Check if this is a clothing/outside question ΓÇö answer from stored weather data
        ...
        if any(
            ...
            # We have weather data ΓÇö A.L.I.C.E answers directly using her own reasoning
            ...
            # ΓöÇΓöÇ Umbrella ΓÇö condition-driven, not temperature-driven ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            if any(
                ...
                # No rain ΓÇö report the facts and let Alice/Ollama advise
                return self._generate_natural_response(
                    {
                        "type": "weather_advice",
                        "temperature": temp,
            ...
            # ΓöÇΓöÇ Clothing items ΓÇö delegate entirely to weather_advice formatter ΓöÇ
            ...
            if any(w in input_lower for w in clothing_items):
                ...
                # "should i wear X or Y?" ΓÇö pick warmer/lighter based on temperature
                _choice_m = re.search(
                ...
                if _choice_m:
                    ...
                    # below 15┬░C ΓåÆ pick the warmer item; at/above 15┬░C ΓåÆ pick the lighter item
                    recommended = (
            ...
            # ΓöÇΓöÇ Cold/warm/hot/freeze condition questions ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            # Report the current conditions ΓÇö let the facts answer the question.
            if any(w in input_lower for w in ["cold", "warm", "hot", "freeze"]):
                if temp is None:
                    return None
                return self._alice_direct_phrase(
                    "weather_report",
                    {
    ...
    def _normalize_weather_intent_for_time_range(
        ...
        if any(cue in text for cue in time_range_cues):
            if hasattr(self, "_think"):
                self._think(
                    f"Weather time-range cue detected ΓåÆ promoting {intent!r} to 'weather:forecast'"
                )
    ...
    def _is_code_access_or_listing_request(self, user_input: str) -> bool:
        ...
        has_scope = has_py_target or any(
            phrase in text for phrase in code_scope_phrases
        )
        if not has_scope:
            return False
    ...
    def _apply_meta_question_override(
        ...
        *,
        ...
        applied = (
            current_intent != target_intent or target_confidence > current_confidence
        )
    ...
    def _apply_weather_domain_override(
        ...
        *,
        ...
        explicit_non_weather_target = bool(
            re.search(
                r"\b(note|notes|memo|email|emails|mail|calendar|event|events|reminder|reminders)\b",
                text,
            )
        )
        ...
        return (
            target_intent,
            max(float(intent_confidence or 0.0), 0.90),
            {
                "applied": True,
    ...
    def _is_answerability_direct_question(self, user_input: str) -> bool:
        ...
        has_direct_structure = any(
            re.search(pat, text) for pat in direct_question_patterns
        )
    ...
    def _retry_llm_answer_after_failure(
        self, *, user_input: str, intent: str
    ) -> Optional[str]:
    ...
    def _safe_llm_failure_response(
        ...
        try:
            ...
            understood_request = (
                force_direct_recovery
                or self._should_answer_first_without_clarification(
                    user_input,
                    intent,
                )
            )
    ...
    def _resolve_turn_success_and_route(
        ...
        *,
        ...
        if llm_attempted:
            ...
            return (
                "llm_fallback" if recovered else "llm",
                bool(generation_ok or recovered),
                recovered,
            )
    ...
    def _apply_confidence_cascade(
        ...
        Confidence cascade policy:
          >= 0.85 ΓåÆ execute directly
          >= 0.65 ΓåÆ execute with a low-confidence prefix marker
          >= 0.45 ΓåÆ ask for clarification
          <  0.45 ΓåÆ surface top-2 interpretations
        """
        ...
        execute_direct_threshold = max(
            execute_direct_threshold, execute_low_conf_threshold
        )
        ...
        if confidence >= execute_low_conf_threshold:
            return {
                ...
                "confidence": confidence,
                "marker": "I'm not 100% sure, but I'll tryΓÇö",
            }
    ...
    def _executive_gate_fallback_response(
        ...
        *,
            ...
            if _recovered:
                self._record_fallback_taxonomy(
                    reason=(
                        _normalized_reason
                        or str(
                            _failure_recovery.get("reason")
                            or "llm_failed_after_answer_directly"
                        )
                    ),
        ...
        _gate_reason = (
            str((getattr(self, "_last_exec_gate_eval", {}) or {}).get("reason") or "")
            .strip()
            .lower()
        )
        ...
        if (
            ...
            and str(fallback_action or "").strip().lower()
            in {"safe_reply", "revise_answer", "clarify"}
            ...
            if refined:
                self._record_fallback_taxonomy(
                    reason=(
                        _normalized_reason or "low_confidence_answer_needs_refinement"
                    ),
                    source="executive_response_gate",
        ...
        if fallback_action == "clarify":
            ...
            if conceptual:
                self._record_fallback_taxonomy(
                    reason=(
                        _normalized_reason or "low_confidence_answer_requiring_retry"
                    ),
                    source="executive_response_gate",
                    action="answer_direct",
                    task_understood=True,
            ...
            if clarification:
                clarify_reason = (
                    _normalized_reason or "clarification_due_to_missing_parameter"
                )
        ...
        if scaffold:
            self._record_fallback_taxonomy(
                reason=(_normalized_reason or "low_confidence_answer_requiring_retry"),
                source="executive_response_gate",
                action="retry",
                task_understood=False,
            )
    ...
    def _executive_apply_response_gate(
        ...
        *,
        ...
        self._think(
            f"Executive response gate ({route}) ΓåÆ score={score:.2f} "
        ...
        if _route_name in {"llm", "llm_fallback"} and _authority is not None:
            ...
            if outcome is not None:
                ...
                if outcome.action == "deterministic_fallback" and outcome.text:
                    self._record_fallback_taxonomy(
                        reason=str(
                            outcome.reason or "llm_rejected_deterministic_fallback"
                        ),
                        source="executive_response_gate",
                        action="answer_direct",
                        task_understood=True,
        ...
        if fallback_reason:
            self._think(f"Executive fallback reason ΓåÆ {fallback_reason}")
        ...
        *,
        ...
        try:
            ...
            if _turn_quality and _turn_quality.failure_type != "none":
                self._think(
                    f"Quality tracker ΓåÆ failure={_turn_quality.failure_type}, "
    ...
    def _contract_think_stage(
        self, *, user_input: str, raw_response: str
    ) -> ReasoningOutput:
    ...
    def _contract_decide_stage(
        ...
        *,
        ...
        context = {
            ...
            "response": candidate,
            "tone": str(
                getattr(getattr(self, "_last_policy", None), "tone", "neutral")
                or "neutral"
            ),
        }
        ...
        if getattr(self, "response_formulator", None):
            ...
            if isinstance(final_payload, UserResponse):
                ...
                logger.warning(
                    "Response contract violation: non-UserResponse emitted from formulator"
                )
    ...
    def _contract_respond_stage(
        ...
        *,
        ...
        if getattr(
            self, "response_formulator", None
        ) and self.response_formulator.is_internal(message):
        ...
        if self._is_answerability_direct_question(
            user_input
        ) and self._looks_like_clarification_fallback(message):
            message = self._answerability_gate_fallback_response(user_input)

        if self._looks_like_clarification_fallback(
            message
        ) and self._should_answer_first_without_clarification(
            user_input, reasoning_output.intent
        ):
            if self._is_project_ideation_turn(
                user_input, reasoning_output.intent
                ...
                message = self._compose_understood_goal_recovery(
                    user_input, reasoning_output.intent
                )
        ...
        if self._is_technical_agent_build_request(
            ...
            self._think(
                "Contract guard -> replaced generic PM advice with agent-build architecture response"
            )
            message = self._agentic_system_build_response(user_input)

        return message

    def _finalize_user_response_contract(
        self, *, user_input: str, raw_response: str
    ) -> str:
    ...
    def _process_input_internal(self, user_input: str, use_voice: bool = False) -> str:
        ...
        try:
            ...
            self.structured_logger.info(
                "Processing input", input_length=len(user_input)
            )
            logger.info(f"User: {user_input}")
            is_location_query = self._is_location_query(user_input)
            bypass_canonical_weather = self._is_weather_clothing_time_range_request(
                user_input
            )
            ...
            if (
                ...
                try:
                    ...
                    if (
                        _pipeline_result
                        and _pipeline_result.handled
                        and _pipeline_result.response_text
                    ):
                        ...
                        self.structured_logger.info(
                            ...
                            verification_reason=str(
                                ((_meta.get("verification") or {}).get("reason")) or ""
                            ),
                        )
                        return _pipeline_result.response_text
                except Exception as e:
                    logger.debug(
                        f"[ContractPipeline] Fallback to legacy path due to: {e}"
                    )

            # ===== DISTRIBUTED CACHE CHECK =====
            # Check if we have a cached response for this exact input
            cache_key = f"response_{hash(user_input)}"
            _allow_distributed_cache = self._should_use_distributed_response_cache(
                user_input
            )
            ...
            if cached_response:
                ...
                self._think("Distributed cache hit ΓåÆ ultra-fast response")
            ...
            if getattr(self, "context_resolver", None):
                ...
                if getattr(self, "conversation_state_tracker", None):
                    try:
                        ...
                        _pre_state["current_topic"] = str(
                            _cs.get("conversation_topic", "") or ""
                        )
                ...
                if getattr(self, "conversation_context", None):
                    try:
                        ...
                        _pre_state["referenced_entities"] = list(
                            _ctx.get("salient_entities", []) or []
                        )
                        _last_file = str(_ctx.get("last_file") or "")
                        _last_concept = str(_ctx.get("last_concept") or "")
                        _last_object = str(_ctx.get("last_object") or "")
                        _pre_state["last_subject"] = (
                            _last_file or _last_concept or _last_object
                        )
                    except Exception:
                        pass

                _last_files = (
                    (getattr(self, "code_context", {}) or {}).get(
                        "last_files_shown", []
                    )
                ...
                _context_resolution = self.context_resolver.resolve(
                    user_input, _pre_state
                )
                    ...
                    return self._generate_natural_response(
                        {
                            ...
                            "pronouns": list(
                                _context_resolution.unresolved_pronouns or []
                            ),
                            "options": list(
                                _context_resolution.clarification_options or []
                            ),
                        },
                        "helpful",
                        None,
            ...
            if _context_resolution:
                _resolution_confidence = float(
                    _context_resolution.rewrite_confidence or 0.0
                )
            ...
            if getattr(self, "nlp", None) and getattr(self.nlp, "context", None):
                ...
                if not _pending_now:
                    _rehydrated_slot = {}
                    _local_slot = dict(
                        getattr(self, "_pending_conversation_slot", {}) or {}
                    )
                    if bool(_local_slot.get("active")) and (
                        _local_slot.get("slot_type") or _local_slot.get("type")
                    ):
                        _rehydrated_slot = _local_slot
                    if not _rehydrated_slot and getattr(
                        self, "conversation_state_tracker", None
                    ):
                        try:
                            _cs = (
                                self.conversation_state_tracker.get_state_summary()
                                or {}
                            )
                            _candidate = dict(
                                _cs.get("pending_followup_slot", {}) or {}
                            )
                            if not _candidate:
                                _candidate = dict(
                                    _cs.get("pending_clarification", {}) or {}
                                )
                            if _candidate:
                            ...
                            pass
                    if not _rehydrated_slot and getattr(
                        self, "world_state_memory", None
                    ):
                        try:
                            _ws = self.world_state_memory.snapshot() or {}
                            _candidate = dict(
                                _ws.get("pending_clarification", {}) or {}
                            )
                            if _candidate:
                                _rehydrated_slot = _candidate
                        except Exception:
            ...
            if _clarification_resolution and _clarification_resolution.consumed:
                _choice = (
                    str(_clarification_resolution.route_choice or "").strip().lower()
                )
                _selected_branch = (
                    str(getattr(_clarification_resolution, "selected_branch", "") or "")
                    .strip()
                    .lower()
                )
                self._internal_reasoning_state["clarification_resolution"] = (
                    _clarification_resolution.as_dict()
                )
                if _choice == "quick_search":
                    _search_query = str(
                        _clarification_resolution.reconstructed_input or ""
                    ).strip()
                    response = self._resolve_quick_search_from_clarification(
                        _search_query
                    )
                ...
                if _reconstructed:
                    self._clear_pending_conversation_slot_state(
                        _choice or _selected_branch or "resolved"
                    )
            ...
            self._internal_reasoning_state["context_resolution_confidence"] = (
            ...
            if (
                self._is_explicit_greeting_input(user_input)
                and not _pending_clarification_now
            ):
                _native_greeting = self._native_scaffold_response(
                    user_input, "greeting"
                )
                if _native_greeting:
                    self._think("Native social fast path ΓåÆ greeting")
                    self._record_fallback_taxonomy(
                    ...
                    self._store_interaction(
                        user_input, _native_greeting, "greeting", {}
                    )
                    if use_voice and self.speech:
            ...
            if (
                ...
                entities = dict(entities or {})
                entities["resolved_bindings"] = dict(
                    _context_resolution.resolved_bindings
                )
            ...
            # ΓöÇΓöÇ 1.1  Perception layer ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            ...
            # ΓöÇΓöÇ 1.2  Interaction policy ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            ...
            policy = self.interaction_policy.derive(
                perception.inferred_mood,
                sentiment,
                urgency_level,
                intent=intent,
                intent_conf=intent_confidence,
                frame_name=getattr(_frame_info, "frame_name", None)
                if _frame_info
                else None,
                frame_conf=getattr(_frame_info, "confidence", 0.0)
                if _frame_info
                else 0.0,
                n_slots=len(getattr(_frame_info, "slots", {}) or {})
                if _frame_info
                else 0,
            ...
            if perception.inferred_mood not in ("neutral", "positive"):
                self._think(
                    f"Mood={perception.inferred_mood!r} ΓåÆ "
                    f"skip_clarification={policy.skip_clarification}, tone={policy.tone!r}"
                )

            # ΓöÇΓöÇ 1.3  Follow-up resolution ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            ...
            # is unavailable.
            _modifiers = (getattr(nlp_result, "parsed_command", {}) or {}).get(
                "modifiers", {}
            )
            _followup_meta = (
                _modifiers.get("followup_resolution")
            ...
            if isinstance(_followup_meta, dict) and _followup_meta.get("was_followup"):
                ...
                _resolved_conf = float(
                    _followup_meta.get("confidence", intent_confidence)
                ...
                self._think(
                    ...
                    f"{intent} ΓåÆ {_resolved_intent} "
                    f"(confidence {intent_confidence:.2f} ΓåÆ {_resolved_conf:.2f})"
                )
            ...
            if not _followup_applied:
                self._think(
                    "No NLP follow-up metadata this turn; continuing without secondary follow-up override."
                )
            ...
            if isinstance(_slot_followup, dict) and _slot_followup.get("filled"):
                ...
                if (
                    _slot_name == "route_choice"
                    or _slot_reason == "pending_route_choice_slot"
                ):
                    _parent_request = str(
                        _slot_followup.get("parent_request")
                        or (getattr(self, "_pending_conversation_slot", {}) or {}).get(
                            "parent_request"
                        )
                        or ""
                    ).strip()
            ...
            _selected_reference = str(
                _modifiers.get("selected_object_reference")
                if isinstance(_modifiers, dict)
                else ""
            ).strip()
            ...
            if self.context_intent_refiner:
                try:
                    _recent_topic = ""
                    if (
                        hasattr(self.nlp, "conversation_memory")
                        and self.nlp.conversation_memory
                    ):
                    ...
                    _new_conf = float(
                        _refined.get("confidence", intent_confidence)
                        or intent_confidence
                    )
                    if (
                        _new_intent != intent
                        or abs(_new_conf - float(intent_confidence or 0.0)) >= 0.01
                    ):
                        self._think(
                            f"Context refine [{_refined.get('reason', 'n/a')}] ΓåÆ {intent!r} ({float(intent_confidence or 0.0):.2f}) "
                ...
                # ΓöÇΓöÇ Slot inheritance ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
                ...
                # in ΓÇö current-turn values always win, prior values fill gaps.
                _prior_entities = (
                    getattr(self.nlp.context, "last_entities", {})
            ...
            if self._is_project_ideation_request(user_input):
                ...
                if intent == "conversation:clarification_needed":
                    self._think(
                        "Project ideation detail detected -> clarification satisfied"
                    )
                    intent = "learning:project_ideation"
                    intent_confidence = max(float(intent_confidence or 0.0), 0.84)
                self._clear_pending_conversation_slot_state("clarification_satisfied")

            # ΓöÇΓöÇ 1.4  Validation / clarification gate ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            # Now validates the *resolved* intent ΓÇö follow-up corrections are
            ...
            if hasattr(self.nlp, "feature_flags") and self.nlp.feature_flags:
                show_clarification = self.nlp.feature_flags.is_enabled(
                    "nlp_validation_prompts"
                )
            else:
            ...
            # Also suppress if follow-up resolution changed the intent ΓÇö the
            # original validation issues no longer apply to the new intent.
            if bool(
                _followup_applied
                or (
                    isinstance(_followup_meta, dict)
                    and _followup_meta.get("was_followup")
                )
            ):
                show_clarification = False
            ...
            has_critical_issue = any(
                "Missing required" in issue for issue in validation_issues
            )
            ...
            if show_clarification and (validation_score < 0.50 and has_critical_issue):
                ...
                if validation_issues:
                    critical_issues = [
                        i for i in validation_issues if "Missing required" in i
                    ]
                    if critical_issues:
            ...
            self._think(f"NLP ΓåÆ intent={intent!r} confidence={intent_confidence}")
            ...
            # ΓöÇΓöÇ 1.5  Reference resolution ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
                ...
                for pronoun in pronouns:
                    if pronoun in words:
                        ...
                        if resolved:
                            ...
                            _intent_domain = (
                                intent.split(":")[0] if ":" in intent else intent
                            )
                            ...
                            if not _skip:
                                self._think(
                                    f"Reference resolution: '{pronoun}' ΓåÆ '{resolved}'"
                                )
                                if "resolved_reference" not in entities:
            ...
            # ΓöÇΓöÇ REMINDER FAST PATH ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            ...
            if intent == "reminder:set":
                ...
                if proactive:
                    ...
                    try:
                        time_str = trigger.strftime("%I:%M %p").lstrip(
                            "0"
                        ) or trigger.strftime("%I:%M %p")
                    except Exception:
                        time_str = str(trigger)
                    response = f"Got it ΓÇö I'll remind you to {subject} at {time_str}."
            ...
            if intent == "reminder:list":
                ...
                if proactive:
                    ...
                    for r in pending[:10]:
                        ...
                        lines.append(f"ΓÇó {r.message}  ({t})")
            ...
            if intent == "reminder:cancel":
                ...
                if proactive:
                    ...
                    if proactive.cancel_reminder(keyword):
                        return (
                            f'Done ΓÇö I\'ve cancelled the reminder about "{keyword}".'
                        )
                    else:
            ...
            # ALICE got something wrong, don't re-invoke a plugin ΓÇö let the LLM
            ...
            if any(
                _input_for_correction == m or _input_for_correction.startswith(m)
                for m in _correction_markers
            ):
                self._think(
                    "Correction/disagreement detected ΓåÆ skipping plugins, using LLM"
                )
            ...
            intent, intent_confidence, _meta_override_early = (
                self._apply_meta_question_override(
                    user_input=user_input,
                    intent=intent,
                    intent_confidence=float(intent_confidence or 0.0),
                )
            ...
            if self._is_issue_report_input(user_input):
                ...
                if any(intent.startswith(domain) for domain in _tool_domains):
                    self._think(
                        "Issue-report text detected ΓåÆ forcing conversational routing"
                    )
                    intent = "conversation:general"
            ...
            if (
                ...
                self._think(
                    "Low-confidence tool intent without action cues ΓåÆ conversational fallback"
                )
                intent = "conversation:general"
            ...
            if self._is_wake_word_only_input(user_input):
                self._think(
                    "Wake-word-only input ΓåÆ concise conversational acknowledgment"
                )
                ...
                if response:
                    self._store_interaction(
                        user_input, response, "conversation:general", entities
                    )
                    if use_voice and self.speech:
                        self.speech.speak(response, blocking=False)
                    logger.info(f"A.L.I.C.E: {response[:100]}...")
            ...
            intent, intent_confidence, _weather_override_early = (
                self._apply_weather_domain_override(
                    user_input=user_input,
                    intent=intent,
                    intent_confidence=float(intent_confidence or 0.0),
                    entities=entities,
                    followup_meta=_followup_meta
                    if isinstance(_followup_meta, dict)
                    else None,
                )
            )
            if _weather_override_early.get("applied"):
                ...
                entities.setdefault(
                    "domain_override",
                    _weather_override_early.get("reason", "weather_domain_override"),
                )
            ...
            _intent_is_non_weather = any(
                intent.startswith(d) for d in _non_weather_domains
            )
            _wfu_hit = not _intent_is_non_weather and (
            ...
            if _wfu_hit:
                weather_followup_early = self._handle_weather_followup(
                    user_input, intent
                )
            if weather_followup_early:
                self._last_had_stored_data = (
                    True  # fast-path used stored data; no plugin needed
                )
                self._think(
                    "Weather follow-up (early) ΓåÆ answering from stored data (no LLM)"
                )
                self._store_interaction(
                    user_input, weather_followup_early, intent, entities
                )
                if use_voice and self.speech:
            ...
            if intent in cacheable_intents:
                ...
                if cached_response:
                    self._think("Cache hit ΓåÆ using stored response")
            ...
            if is_pure_conversation:
                self._think(
                    "Fast path: pure conversation ΓåÆ trying conversational engine"
                )

                # Try conversational engine first (no plugins, no LLM unless needed)
                if (
                    hasattr(self, "conversational_engine")
                    and self.conversational_engine
                ):
                    context = ConversationalContext(
                        user_input=user_input,
                        intent=intent,
                        entities=entities,
                        recent_topics=self.conversation_topics[-3:]
                    ...
                    if self.conversational_engine.can_handle(
                        ...
                        self._think(
                            "Conversational engine ΓåÆ no learned pattern, using native greeting/fallback path"
                        )
                ...
                high_confidence_greeting_lock = (
                    intent == "greeting" and float(intent_confidence or 0.0) >= 0.82
                )
                if (
                    intent == "greeting"
                    and not explicit_greeting_input
                    and not high_confidence_greeting_lock
                ):
                    self._think(
                        "Intent labeled greeting, but input is not explicit greeting ΓåÆ continue normal routing"
                    )
                    intent = "conversation:general"
                    intent_confidence = min(intent_confidence, 0.55)
                elif (
                    intent == "greeting"
                    and high_confidence_greeting_lock
                    and not explicit_greeting_input
                ):
                    self._think(
                        "High-confidence greeting intent locked despite imperfect greeting spelling"
                    )

                if intent == "greeting" and (
                    explicit_greeting_input or high_confidence_greeting_lock
                ):
                    ...
                    if (
                        ...
                        if self.conversational_engine.can_handle(
                            user_input, intent, conv_context
                        ):
                            response = self.conversational_engine.generate_response(
                                conv_context
                            )
                            if response:
                                self._think(
                                    "Greeting ΓåÆ using learned pattern (no LLM)"
                                )
                                self._cache_put(user_input, intent, response)
                                self._store_interaction(
                                    user_input, response, intent, entities
                    ...
                    native_greeting = self._native_scaffold_response(
                        user_input, "greeting"
                    )
                    if native_greeting:
                        self._cache_put(user_input, intent, native_greeting)
                        self._store_interaction(
                            user_input, native_greeting, intent, entities
                        )
                        if use_voice and self.speech:
                    ...
                    final_greeting = self._dynamic_short_greeting(
                        ...
                        user_name=(
                            getattr(
                                getattr(self, "context", None), "user_prefs", None
                            ).name
                            if getattr(
                                getattr(self, "context", None), "user_prefs", None
                            )
                    ...
                    if final_greeting:
                        ...
                        self._store_interaction(
                            user_input, final_greeting, intent, entities
                        )
                        if use_voice and self.speech:
            ...
            self.last_nlp_result = vars(
                nlp_result
            ).copy()  # Convert to dict for compatibility
            ...
            _old_domain = (
                _prev_intent.split(":")[0] if ":" in _prev_intent else _prev_intent
            )
            if _new_domain != _old_domain:
                self._last_intent_turn = getattr(
                    getattr(self.nlp, "context", None), "turn_index", 0
                )
            ...
            if (
                hasattr(self, "context_cache")
                and self.last_intent
                and intent != self.last_intent
            ):
                # Only invalidate if it's a major topic shift
                if not any(
                    word in intent for word in self.last_intent.split(":")
                ) and not any(
                    word in self.last_intent.split(":") for word in intent.split(":")
                ):
                    self.context_cache.invalidate()
                    self._think("Context cache invalidated (topic shift)")

            # 1.5. Apply Active Learning improvements
            improved_nlp_result = self.learning_manager.apply_learning(
                user_input, vars(nlp_result)
            )
            if improved_nlp_result != vars(nlp_result):
                ...
                self._think(f"     active learning ΓåÆ intent={intent!r}")
                ...
                if intent != original_intent_before_al:
                    self.nlp_error_logger.log_intent_override(
                        ...
                        corrected_confidence=improved_nlp_result.get(
            ...
            # ΓöÇΓöÇ BayesianIntentRouter: minimise expected regret across candidates ΓöÇ
            ...
            try:
                if getattr(self, "bayesian_router", None) is not None and intent:
                    ...
                    for _alt_plugin, _alt_score in sorted(
                        ...
                        if (
                            _alt_plugin_prefix == _primary_plugin
                            and _alt_intent.endswith(":general")
                        ):
                            continue
                    ...
                    _rd = self.bayesian_router.decide(
                        _candidates,
                        user_priors=getattr(
                            self.user_profile, "get_intent_priors", lambda: {}
                        )(),
                    )
                    if _rd.intent != intent:
                        # Never downgrade a high-confidence specific intent to
                        # a bare `:general` catch-all ΓÇö let the NLP win.
                        _is_downgrade = (
                        ...
                        if not _is_downgrade:
                            self._think(
                                f"BayesianRouter overrides {intent!r} ΓåÆ {_rd.intent!r} "
            ...
            try:
                if self.world_graph is not None:
                    ...
                    _wg_topics = (
                        list(self.conversation_topics[-5:])
                        if self.conversation_topics
                        else []
                    )
            ...
            advanced_reasoning_response = self._handle_advanced_reasoning_queries(
                user_input
            )
            ...
            if code_response:
                self._think("Code request detected ΓåÆ handled directly")
                ...
                self._store_interaction(
                    effective_input, code_response, _code_intent, _code_entities
                )
            ...
            if training_response:
                self._think("Training request detected ΓåÆ handled directly")
            ...
            if getattr(self, "conversational_engine", None):
                conv_context = ConversationalContext(
                    ...
                    intent=intent,
                    entities=entities or {},
                    recent_topics=self.conversation_topics[-5:]
                    if self.conversation_topics
                    else [],
                    active_goal=goal_res.goal.description
                ...
                if self.conversational_engine.can_handle(
                    ...
                    self._think(
                        "Checking if A.L.I.C.E has learned patterns for this..."
                    )
                    response = self.conversational_engine.generate_response(
                        conv_context
                    )
                    if response:
                        self._think(
                            "A.L.I.C.E responded from learned patterns (no Ollama)"
                        )
                        self._store_interaction(user_input, response, intent, entities)
                        return response
                    else:
                        self._think("No learned patterns yet ΓåÆ will use Ollama")
            ...
            weather_followup = (
                None if _wfu_hit else self._handle_weather_followup(user_input, intent)
            )
            if weather_followup:
                self._think("Weather follow-up ΓåÆ answering from stored data (no LLM)")
                ...
                if turn.entities_resolved:
                    for reference, entity_id in turn.entities_resolved.items():
                        ...
                        if entity:
                            ...
                            context_resolved_input = user_input.replace(
                                reference, entity_desc
                            )
            ...
            if hasattr(self, "reasoning_engine") and self.reasoning_engine:
                try:
                    ...
                    if resolution.bindings:
                        self._think(
                            f"Reference resolution ΓåÆ {list(resolution.bindings.keys())} ΓåÆ {[r.resolved_label for r in resolution.bindings.values()]}"
                        )
                        user_input_processed = (
                            resolution.resolved_input or user_input_processed
                        )
                    ...
                    if goal_res.cancelled and goal_res.message:
                        ...
                        if intent in cancel_ack_intents:
                            self._think("Goal cancelled ΓåÆ returning ack")
                            return goal_res.message
                        self._think(
                            f"Goal cancelled ΓåÆ continuing with intent={intent!r}"
                        )
                    if goal_res.revised:
                        _revised_intent = str(goal_res.intent or intent)
                        _revised_entities = dict(goal_res.entities or entities or {})
                        _project_detail_satisfied = self._is_project_ideation_request(
                            user_input
                        )
                        if (
                            _project_detail_satisfied
                            and _revised_intent == "conversation:clarification_needed"
                        ):
                            self._think(
                                "Goal revise suggested clarification, but detail is now satisfied -> preserving ideation intent"
                            )
                            intent = "learning:project_ideation"
                            entities = {**(entities or {}), **_revised_entities}
                            entities.setdefault("goal", str(user_input or "").strip())
                            entities.setdefault(
                                "user_goal", str(user_input or "").strip()
                            )
                            entities.setdefault(
                                "objective", str(user_input or "").strip()
                            )
                            intent_confidence = max(
                                float(intent_confidence or 0.0), 0.84
                            )
                            self._clear_pending_conversation_slot_state(
                            ...
                            self._think(f"Goal revised ΓåÆ intent={intent!r}")
                    elif goal_res.goal:
            ...
            if _goal_followup_resolution.get("matched"):
                ...
                _goal_desc = str(
                    _goal_record.get("description") or _goal_record.get("title") or ""
                ).strip()
                _goal_intent = str(
                    _goal_followup_resolution.get("intent_hint") or ""
                ).strip()
                ...
                if _goal_id:
                    ...
                    entities.setdefault(
                        "goal_title", str(_goal_record.get("title") or _goal_desc)
                    )
                    entities.setdefault(
                        "goal_status", str(_goal_record.get("status") or "active")
                    )
                if _goal_entities:
                ...
                _reconstructed_goal_input = str(
                    _goal_followup_resolution.get("reconstructed_input") or ""
                ).strip()
            ...
            # Handle reply confirmation
                ...
                if user_input.lower() in ["yes", "y", "send", "confirm"]:
                    if self.gmail and self.gmail.reply_to_email(
                        self.pending_data["email_id"], self.pending_data["reply_body"]
                    ):
                        result = "Reply sent!"
                        # Clear pending action
                        self.pending_action = None
            ...
            try:
                ...
                _pre_scores = self.executive_controller.score_decisions(
                    ...
                    has_explicit_action_cue=bool(
                        self._has_explicit_action_cue(user_input_processed)
                    ),
                    has_active_goal=bool(_pre_goal_stack),
                    force_plugins_for_notes=False,
                )
                _plan_allowed = self.executive_controller.should_use_planner(
                    _pre_exec_state, _pre_scores
                )
                if _plan_allowed:
                    planned_response = self._use_planner_executor(
                        intent, entities, user_input_processed
                    )
                    if planned_response:
                        self._internal_reasoning_state = {
            ...
            if (
                number_match
                and self.last_email_list
                and any(
                    word in user_input.lower()
                    for word in ["delete", "remove", "read", "open", "archive", "mark"]
                )
            ):
                ...
                if 1 <= email_num <= len(self.last_email_list):
                    ...
                    if "delete" in query_lower or "remove" in query_lower:
                        if self.gmail and self.gmail.delete_email(email["id"]):
                            self.last_email_list[email_num - 1] = (
                                None  # Mark as deleted, don't shift list
                            )
            ...
            if intent and intent.startswith("email"):
                ...
                # Check/List emails
                if any(
                    word in query_lower for word in ["check", "show", "list", "inbox"]
                ):
                    if "unread" in query_lower:
                        # Count unread
                        count = self.gmail.get_unread_count()
                        return (
                            f"You have {count} unread email{'s' if count != 1 else ''}."
                        )
                    ...
                    for i, email in enumerate(emails, 1):
                        ...
                        sender_name = (
                            from_field.split("<")[0].strip().strip('"')
                            if "<" in from_field
                            else from_field
                        )
                        # Simplify date - just the date part
                        date_str = (
                            email["date"].split(",", 1)[1].strip().split(" +")[0]
                            if "," in email["date"]
                            else email["date"]
                        )

                        response += f"{i}. {unread}{email['subject']}\n"
                        response += f"   {sender_name} ΓÇó {date_str}\n\n"

                    return response.strip()

                # Read specific email(s) or latest
                elif any(
                    word in query_lower for word in ["read", "open", "first", "latest"]
                ):
                    ...
                    if requested_count and requested_count > 1:
                        emails = self.gmail.get_recent_emails(
                            max_results=min(requested_count, 10)
                        )
                        if not emails:
                    ...
                    if "from" in query_lower:
                        ...
                        try:
                            ...
                            if from_idx + 1 < len(words):
                                ...
                                emails = self.gmail.get_emails_by_sender(
                                    sender, max_results=5
                                )
                ...
                # Send/Write email
                    ...
                    word in query_lower
                    for word in ["send", "write", "compose", "draft"]
                ):
                ...
                # Delete email
                    ...
                    if any(
                        word in query_lower
                        for word in ["this", "that", "it", "the email", "the latest"]
                    ):
                        # Advanced context resolution
                        if self.advanced_context:
                            resolved_ref = self.advanced_context.resolve_reference(
                                user_input
                            )
                            if resolved_ref and resolved_ref.startswith("email_"):
                                # Get entity data
                                entity = self.advanced_context.entities.get(
                                    resolved_ref
                                )
                    ...
                    if len(emails) == 1:
                        ...
                        self.last_email_list = emails
                        response = (
                            f"Found {len(emails)} emails matching '{search_terms}':\n\n"
                        )
                        for i, email in enumerate(emails, 1):
                            sender_name = (
                                email["from"].split("<")[0].strip().strip('"')
                ...
                # Reply to email
                    ...
                    if (
                        email_ref
                        and self.last_email_list
                        and 0 < email_ref <= len(self.last_email_list)
                    ):
                ...
                # Unstar/unflag email
                    ...
                    "unstar" in query_lower
                    or "unflag" in query_lower
                    or "remove star" in query_lower
                ):
                    # Look for email reference number
                    email_ref = None
                    for word in user_input.split():
            ...
            active_goal = (
                goal_res.goal
                if (goal_res and goal_res.goal)
                else _goal_followup_goal_object
            )
            if active_goal is None and _authoritative_goal_stack:
                ...
                active_goal = ReasoningActiveGoal(
                    ...
                    description=str(
                        _g.get("description") or _g.get("title") or "active goal"
                    ),
            ...
            intent_plausibility = float(
                getattr(nlp_result, "intent_plausibility", 1.0) or 1.0
            )
            _rich_conceptual_prompt = self._is_rich_conceptual_request(user_input)
            _conceptual_build_prompt = self._is_conceptual_build_architecture_prompt(
                user_input
            )

            if (
                _rich_conceptual_prompt
                and intent == "conversation:clarification_needed"
            ):
                self._think(
                    "Rich conceptual build prompt detected ΓåÆ overriding clarification intent"
                )
                intent = (
                    "learning:system_design"
                    if _conceptual_build_prompt
            ...
            if (
                ...
                and str(active_goal.intent or "").lower().strip()
                != "conversation:clarification_needed"
                and self._should_reuse_goal_intent(user_input, active_goal.description)
            ):
                self._think(
                    f"Low confidence ({intent_confidence:.2f}) but active goal ΓåÆ using goal intent: {active_goal.intent}"
                )
                intent = active_goal.intent
                entities = {
                    **(entities if entities else {}),
                    **(active_goal.entities if active_goal.entities else {}),
                }
                intent_confidence = 0.8
            else:
                if active_goal and intent_confidence < 0.7 and _rich_conceptual_prompt:
                    self._think(
                        "Rich conceptual prompt ΓåÆ forcing fresh intent, not reusing goal intent"
                    )
                elif (
                    active_goal
                    and intent_confidence < 0.7
                    ...
                    self._think(
                        "Active goal is clarification_needed ΓåÆ not reusing goal intent"
                    )
                elif (
                    active_goal
                    and intent_confidence < 0.7
                    and not self._should_reuse_goal_intent(
                        user_input, active_goal.description
                    )
                ):
                    self._think(
                        "Topic shift (low overlap with goal) ΓåÆ not reusing goal intent"
                    )
                elif (
                    active_goal
                    and intent_confidence < 0.7
                    ...
                    self._think("Low plausibility detected ΓåÆ not reusing goal intent")

            intent, intent_confidence, _meta_override_late = (
                self._apply_meta_question_override(
                    user_input=user_input,
                    intent=intent,
                    intent_confidence=float(intent_confidence or 0.0),
            ...
            if _meta_override_late.get("applied"):
                ...
                entities.setdefault(
                    "domain_override",
                    _meta_override_late.get(
                        "reason", "meta_question_identity_override"
                    ),
                )

            intent, intent_confidence, _weather_override_late = (
                self._apply_weather_domain_override(
                    user_input=user_input,
                    intent=intent,
                    intent_confidence=float(intent_confidence or 0.0),
            ...
            if _weather_override_late.get("applied"):
                ...
                entities.setdefault(
                    "domain_override",
                    _weather_override_late.get("reason", "weather_domain_override"),
                )
            ...
            has_note_context = (
                bool(getattr(notes_plugin, "last_note_id", None))
                if notes_plugin
                else False
            )
            force_plugins_for_note_followup = bool(
                has_note_context
                and re.search(
                    r"\b(?:it|this|that|the\s+note|my\s+note)\b", user_input.lower()
                )
                and re.search(
                    r"\b(?:what|read|show|in|inside|content|contents|contains?)\b",
                    user_input.lower(),
                )
            )
            force_plugins_for_explicit_note_read = bool(
                re.search(
                    r"\b(?:read|show|open|what(?:'s|\s+is)?|inside|content|contents?)\b",
                    user_input.lower(),
                )
                and re.search(r"\bnotes?\b", user_input.lower())
            )
            force_plugins_for_notes = (
                force_plugins_for_note_followup or force_plugins_for_explicit_note_read
            )
                ...
                _conversation_state = {
                    **(_conversation_state or {}),
                    ...
                    "conversation_goal": str(
                        _authoritative_goal_stack[0].get("status")
                        or _conversation_state.get("conversation_goal")
                        or ""
                    ).strip(),
                    "user_goal": str(
                        _authoritative_goal_stack[0].get("description")
                        or _authoritative_goal_stack[0].get("title")
                        or _conversation_state.get("user_goal")
                        or ""
                    ).strip(),
                }
            ...
            if getattr(self, "cognitive_orchestrator", None):
                try:
                    _cognitive_guidance = (
                        self.cognitive_orchestrator.get_runtime_guidance()
                    )
                except Exception:
                    _cognitive_guidance = {}
            ...
            if _cognitive_guidance:
                ...
                if _penalty > 0.0:
                    intent_confidence = max(
                        0.0, float(intent_confidence or 0.0) - _penalty
                    )
                    self._think(
                        f"Immediate feedback penalty applied ({_penalty:.2f}) ΓåÆ confidence={intent_confidence:.2f}"
                    )
                ...
                _conversation_state = {
                    **(_conversation_state or {}),
                    ...
                    "pending_followup_slot_name": str(
                        _pending_slot_state.get("slot") or ""
                    ),
            ...
            if getattr(self, "world_state_memory", None):
                try:
                    _world_state_before_route = dict(
                        self.world_state_memory.snapshot() or {}
                    )
                except Exception:
                    _world_state_before_route = {}
            ...
            if getattr(self, "turn_state_assembler", None):
                try:
                    ...
                    _hidden_state_snapshot = self.turn_state_assembler.build(
                        ...
                        action_context={
                            ...
                            "selected_reference": str(
                                ...
                                or (_pending_clarification or {}).get(
                                    "selected_reference"
                                )
                                or ""
                            ).strip(),
                    ...
                    _hidden_state_summary = self.turn_state_assembler.to_text(
                        _hidden_state_snapshot
                    )
                except Exception:
                    _hidden_state_snapshot = {}
            ...
            _entities_for_exec["_intent_plausibility"] = float(
                intent_plausibility or 0.0
            )
            ...
            if getattr(self, "response_planner", None):
                try:
                    ...
                    if (
                        getattr(_pre_response_plan, "response_type", "")
                        in ("instruction", "debugging", "troubleshooting")
                        and _has_action_cue
                    ):
            ...
            _pre_route_guard = (
                self.executive_controller.should_preempt_for_plausibility(
                    _executive_state,
                    has_explicit_action_cue=_has_action_cue,
            ...
            if isinstance(_followup_meta, dict) and _followup_meta.get("was_followup"):
                _followup_domain = (
                    str(_followup_meta.get("domain") or "").strip().lower()
                )
            ...
            if _pre_route_guard.get("block") and str(
                (_agentic_authority or {}).get("action") or ""
            ).strip().lower() in {"use_llm", "use_plugin"}:
                _pre_route_guard = {
                    "block": False,
                    "reason": "agentic_primary_authority_bypass",
                }
                self._think(
                    "Pre-route plausibility bypassed (agentic primary authority)"
                )
            ...
            if _pre_route_guard.get("block"):
                _pre_block_decision = ExecutiveDecision(
                    ...
                    clarification_question=str(
                        _pre_route_guard.get("question") or ""
                    ).strip(),
                )
                _pre_block_state_machine = (
                    self.executive_controller.run_turn_state_machine(
                ...
                self._internal_reasoning_state = {
                    **_executive_state.as_dict(),
                    ...
                    "context_resolution_confidence": float(
                        getattr(_context_resolution, "rewrite_confidence", 0.0)
                        if _context_resolution
                        else 0.0
                    ),
                    "intent_candidates": intent_candidates,
                ...
                if _turn_reasoning_plan is not None:
                    self._internal_reasoning_state["reasoning_planner"] = (
                        _turn_reasoning_plan.as_dict()
                    )
                self._think(
                    f"Pre-route plausibility guard ΓåÆ {_pre_route_guard.get('reason', 'unknown')}"
                )
            ...
            if _pre_response_plan is not None:
                self._internal_reasoning_state["response_plan"] = (
                    _pre_response_plan.as_dict()
                )
            ...
            _agentic_action = (
                str((_agentic_authority or {}).get("action") or "").strip().lower()
            )
            _agentic_reason = str(
                (_agentic_authority or {}).get("reason") or "agentic_primary"
            ).strip()
            if _agentic_action in {"use_plugin", "use_llm", "ask_clarification"}:
                _clarify_question = str(
                    (_agentic_authority or {}).get("question") or ""
                ).strip()
                _executive_decision = ExecutiveDecision(
                    action=(
                        "use_plugin"
                        if _agentic_action == "use_plugin"
                        else (
                            "ask_clarification"
                            if _agentic_action == "ask_clarification"
                            else "use_llm"
                        )
                    ),
                    reason=f"agentic_primary:{_agentic_reason}",
                    store_memory=bool(
                        getattr(_executive_decision, "store_memory", True)
                    ),
                ...
                self._think(
                    f"Agentic primary authority ΓåÆ {_executive_decision.action} "
                    f"({_executive_decision.reason})"
                )

            _learning_decision = self.executive_controller.decide_learning(
                relevance=max(
                    _decision_scores.get("llm", 0.0),
                    _decision_scores.get("tools", 0.0),
                    _decision_scores.get("search", 0.0),
                ),
                confidence=float(intent_confidence or 0.0),
                novelty=0.70
                if (_executive_state.topic or _executive_state.user_goal)
                else 0.40,
                risk=0.60
                if _executive_decision.action in ("ask_clarification", "ignore")
                else 0.20,
            )
            self._internal_reasoning_state["learning_decision"] = _learning_decision
            self._exec_should_store_memory = _learning_decision in (
                "store",
                "temporary",
            ) and bool(_executive_decision.store_memory)
            ...
            self._internal_reasoning_state["turn_state_machine"] = (
                _turn_state_machine.as_dict()
            )
            self._internal_reasoning_state["turn_contract"] = (
                _turn_state_machine.contract.as_dict()
            )
            ...
            if getattr(self, "response_planner", None):
                try:
                    ...
                    self._think(
                        f"Response plan ΓåÆ type={_resp_plan.response_type}, "
                        f"strategy={_resp_plan.strategy}"
                    )
                    if (
                        _resp_plan.strategy == "ask_guiding_question"
                        and _turn_state_machine.chosen_route != "tool"
                    ):
                        return self.response_planner.guiding_question(
                            _resp_plan, user_input
                        )
                except Exception as _rp_err:
                    logger.debug(f"[ResponsePlanner] {_rp_err}")

            self._think(
                f"Executive decision ΓåÆ {_executive_decision.action} "
                f"({_executive_decision.reason})"
            )
            self._think(
                "Executive scores ΓåÆ "
                + ", ".join(
                    f"{k}:{v:.2f}"
                    for k, v in sorted(
                        _decision_scores.items(), key=lambda kv: kv[1], reverse=True
                    )[:4]
                )
            )

            if getattr(self, "execution_journal", None) and getattr(
                self, "world_state_memory", None
            ):
                try:
                ...
                if self._self_answer_first_can_override(_executive_decision.action):
                    ...
                    if _self_answer.get("block_llm"):
                        ...
                        if response:
                            ...
                            self._think(
                                f"Self-answer-first gate ΓåÆ {_self_answer.get('reason', 'native')}"
                            )
                            self._cache_put(user_input, intent, response)
                            self._store_interaction(
                                user_input, response, intent, entities
                    ...
                    self._think(
                        "Self-answer-first gate bypassed ΓåÆ llm_authority_lock"
                    )
            ...
            if _executive_decision.action == "ask_clarification":
                if self._should_answer_first_without_clarification(
                    ...
                    self._think(
                        "Executive clarification overridden ΓåÆ answer-first conversational path"
                    )
                    ...
                    if _fast_direct:
                        self._store_interaction(
                            user_input, _fast_direct, intent, entities
                        )
                        if use_voice and self.speech:
                            self.speech.speak(_fast_direct, blocking=False)
                        logger.info(
                            f"A.L.I.C.E (clarification-override): {_fast_direct[:100]}..."
                        )
                        return _fast_direct
            ...
            if _tool_veto.get("veto"):
                ...
                self._think(f"Executive veto ΓåÆ {_tool_veto.get('reason', 'unknown')}")
            ...
            if (
                str(intent or "").lower().strip() == "conversation:meta_question"
                and _should_try_plugins
            ):
                self._think("Meta-question conversation lock ΓåÆ skipping plugin route")
            ...
            if getattr(self, "roadmap_stack", None):
                ...
                if not _route_ok and _should_try_plugins:
                    self._think(
                        f"Route-contract guard switched to clarify path: {_route_reason}"
                    )
                    _should_try_plugins = False
            ...
            _turn_state_machine = self.executive_controller.run_turn_state_machine(
                ...
                has_active_goal=bool(_authoritative_goal_stack),
                pre_route_blocked=False,
                tool_vetoed=not bool(_should_try_plugins)
                and _executive_decision.action == "use_plugin",
            )
            ...
            if not _should_try_plugins:
                if is_pure_conversation:
                    self._think("Pure conversation ΓåÆ skipping plugins, using LLM")
                else:
                    self._think(
                        "Short follow-up (no command words, no goal) ΓåÆ skipping plugins, using LLM"
                    )
            else:
                self._think(
                    f"Trying plugins... (confidence: {intent_confidence:.2f}, goal: {active_goal.description[:30] if active_goal else 'none'}...)"
                )
            if _should_try_plugins:
                intent, intent_confidence, _weather_override_pre_plugin = (
                    self._apply_weather_domain_override(
                        user_input=user_input,
                        intent=intent,
                        intent_confidence=float(intent_confidence or 0.0),
                ...
                if _weather_override_pre_plugin.get("applied"):
                    ...
                    entities.setdefault(
                        "domain_override",
                        _weather_override_pre_plugin.get(
                            "reason", "weather_domain_override"
                        ),
                    )
                ...
                if getattr(self, "roadmap_stack", None):
                    _allowed, _reason = self.roadmap_stack.secondary_safety.validate(
                        ...
                        has_approval=(
                            "confirm" in user_input.lower()
                            or "operator approve" in user_input.lower()
                        ),
                    )
                    if not _allowed:
                ...
                if getattr(self, "rbac_engine", None):
                    ...
                    if (
                        _rbac_decision.allowed
                        and not _rbac_decision.requires_confirmation
                    ):
                        if (
                            getattr(self, "approval_ledger", None)
                            and "confirm" in user_input.lower()
                            and getattr(_rbac_decision, "required_scope", None)
                            is not None
                        ):
                    ...
                    if _rbac_decision.requires_confirmation:
                        if (
                            ...
                            and getattr(_rbac_decision, "required_scope", None)
                            is not None
                            and self.approval_ledger.is_scope_approved(
                                _rbac_decision.required_scope.value
                            )
                        ):
                            self._think(
                                "Approval ledger scope-memory satisfied confirmation requirement"
                            )
                        else:
                            self._think(
                                "RBAC requires explicit confirmation for this action"
                            )
                ...
                if nlp_result is not None:
                    context_summary["nlp"] = {
                        ...
                        "intent_candidates": intent_candidates,
                        "intent_plausibility": float(intent_plausibility or 0.0),
                        "parsed_command": getattr(nlp_result, "parsed_command", {})
                        or {},
                        "plugin_scores": getattr(nlp_result, "plugin_scores", {}) or {},
                        "token_debug": getattr(nlp_result, "token_debug", [])[:30],
                        "dialogue_memory": self.dialogue_memory.entity_chain_dict()
                ...
                if _schema_error:
                    self._think(
                        f"Tool invocation schema validation blocked dispatch: {_schema_error}"
                    )
                ...
                _risk_scope = (
                    getattr(_rbac_decision, "required_scope", None)
                    if "_rbac_decision" in locals()
                    else None
                )
                _risk_scope_value = str(
                    getattr(_risk_scope, "value", _risk_scope) or ""
                ).lower()
                ...
                _expected_outcome = (
                    ...
                    if _contract_success_criteria
                        ...
                        if active_goal
                        else f"Complete {_action_name} successfully"
                    )
                )
                ...
                action_request = ActionRequest(
                    ...
                    retry_budget=(
                        1 if _action_name in {"read", "list", "search"} else 0
                    ),
                    rollback_policy=(
                        "manual"
                        if _action_name in {"delete", "update", "append"}
                        else "none"
                    ),
                    goal_ref=(
                        {
                            ...
                            "status": str(
                                getattr(
                                    getattr(active_goal, "status", ""),
                                    "value",
                                    getattr(active_goal, "status", "active"),
                                )
                                or "active"
                            ),
                            "next_action": str(
                                getattr(active_goal, "next_action", "") or ""
                            ),
                            "confidence": float(
                                getattr(
                                    active_goal, "confidence", intent_confidence or 0.0
                                )
                                or 0.0
                            ),
                        }
                ...
                if plugin_result:
                    _plugin_result_name = (
                        str(plugin_result.get("plugin") or "").strip().lower()
                    )
                    _plugin_result_success = bool(plugin_result.get("success", False))
                    _reroute_intent, _reroute_conf, _weather_plugin_guard = (
                        self._apply_weather_domain_override(
                            user_input=user_input,
                            intent=intent,
                            intent_confidence=float(intent_confidence or 0.0),
                    ...
                    if (
                        ...
                        intent_confidence = max(
                            float(intent_confidence or 0.0), float(_reroute_conf or 0.0)
                        )
                        entities = dict(entities or {})
                        entities.setdefault(
                            "domain_override",
                            _weather_plugin_guard.get(
                                "reason", "weather_domain_override"
                            ),
                        )
                        ...
                        _reroute_request = ActionRequest(
                            goal=(
                                active_goal.description if active_goal else user_input
                            ),
                ...
                if (
                    action_result.goal_satisfied
                    and active_goal
                    and getattr(self, "goal_system", None)
                ):
                    try:
                ...
                _recovery_outcome = str(
                    getattr(action_result, "recovery_path", "") or ""
                ).strip()
                if _recovery_outcome and getattr(
                    self, "conversation_state_tracker", None
                ):
                    try:
                        self.conversation_state_tracker.set_last_recovery_outcome(
                            _recovery_outcome
                        )
                    except Exception:
                        pass
                if _recovery_outcome and getattr(self, "world_state_memory", None):
                    try:
                        self.world_state_memory.set_last_recovery_outcome(
                            _recovery_outcome
                        )
                    except Exception:
                        pass
                ...
                if _recommended_next in {"retry", "clarify_then_continue", "escalate"}:
                    self._internal_reasoning_state["goal_verifier_next_action"] = (
                        _recommended_next
                    )
                ...
                if plugin_result:
                    _verification = (
                        action_result.state_updates.get("verification", {}) or {}
                    )
                    if getattr(self, "roadmap_stack", None):
                ...
                if plugin_result:
                    ...
                    self.metrics.track_plugin(
                        plugin_name, action, plugin_duration, success
                    )
                    self.structured_logger.debug(
                ...
                if plugin_result:
                    ...
                    _p_verified = bool(
                        (plugin_result.get("verification") or {}).get("accepted", False)
                    )
                    ...
                    try:
                        if self.capability_graph is not None and _p_verified:
                            self.capability_graph.record_execution(
                                _p_name, intent, _p_success
                            )
                    except Exception:
                        pass
                    try:
                ...
                if not plugin_result and intent_confidence < 0.6:
                    self._think(
                        "Low confidence + no plugin match ΓåÆ using LLM with context"
                    )
            ...
            if plugin_result:
                ...
                self._think(f"Plugin ΓåÆ {plugin_name} success={success}")
                ...
                if not _tool_verified:
                    self._think(
                        "Tool output not verified ΓåÆ clarification before state updates"
                    )
                    response = (
                    ...
                    self._internal_reasoning_state["task_verification"] = (
                        _contract_verification.to_dict()
                    )
                ...
                context = ConversationalContext(
                    ...
                    plugin_data={
                        **(
                            plugin_result.get("data", {})
                            if isinstance(plugin_result.get("data", {}), dict)
                            else {}
                        ),
                ...
                if plugin_result.get("formulate", False):
                    self._think(
                        "Plugin requested formulation ΓåÆ Alice will learn to respond"
                    )

                # Foundation System Integration: Context-aware response generation
                if self.foundations and self.foundation_mode in [
                    "primary",
                    "exclusive",
                ]:
                    try:
                        self._think(
                            f"Foundation systems active (mode={self.foundation_mode}) ΓåÆ generating context-aware response"
                        )
                        ...
                        self._think(
                            f"Foundation response generated: {response[:100] if response else 'None'}..."
                        )
                    except Exception as e:
                    ...
                    try:
                        self._think(
                            "Foundation parallel mode ΓåÆ running both systems for comparison"
                        )
                        foundation_result = self.foundations.process_interaction(
                            user_id=runtime_user_name,
                            user_input=user_input,
                        ...
                        logger.info(
                            f"Foundation (parallel): {foundation_response[:100] if foundation_response else 'None'}..."
                        )
                    except Exception as e:
                ...
                if (
                    alice_formulation
                    and alice_formulation.get("type") == "self_analysis_needed"
                ):
                    self._think("Self-analysis requested ΓåÆ reading actual codebase")
                    ...
                    for file_path in key_files:
                        try:
                            file_info = self.self_reflection.analyze_file_advanced(
                                file_path
                            )
                ...
                if (
                    alice_formulation
                    and alice_formulation.get("type") != "general_response"
                ):
                    ...
                    try:
                        ...
                        self._think(
                            "Alice formulated and phrased response using tool-based architecture"
                        )
                    except Exception as e:
                        logger.error(f"Error in tool-based response generation: {e}")
                        response = None

                # Fallback: Alice phrases directly ΓÇö no Ollama
                if not response:
                    self._think(
                        "Response generation failed ΓåÆ Alice uses direct phrase fallback (no Ollama)"
                    )
                    if alice_formulation and alice_formulation.get("type") not in (
                        "general_response",
                        None,
                    ):
                        # Try Alice's structured direct-phrase engine first
                        response = self._alice_direct_phrase(
                            alice_formulation.get("type", ""), alice_formulation
                        )
                    if not response:
                        # Simple inline Alice formatter ΓÇö no LLM involved
                        action = (
                            plugin_result.get("action", intent)
                            if isinstance(plugin_result, dict)
                            else intent
                        )
                        data = (
                        ...
                        if success:
                            ...
                            response = (
                                "Done." if not subject else f"Done ΓÇö {subject}."
                            )
                        else:
                ...
                if getattr(self, "response_optimizer", None):
                    response = self.response_optimizer.optimize(
                        response, intent, {"plugin": plugin_name}
                    )
                ...
                if _secondary_intents:
                    ...
                    if _sec_outcomes and self.multi_step_reasoning_engine:
                        _sec_summary = (
                            self.multi_step_reasoning_engine.summarize_outcomes(
                                _sec_outcomes
                            )
                        )
                        if _sec_summary:
                ...
                if _turn_execution_outcome is not None:
                    ...
                    if not _turn_execution_outcome.verification_passed and bool(
                        plugin_result.get("retryable", False)
                    ):
                ...
                if (
                    _post_execution
                    and _post_execution.contract
                    and _turn_execution_outcome is not None
                ):
                ...
                if (
                    _turn_execution_outcome is not None
                    and not _turn_execution_outcome.verification_passed
                ):
                    self._think(
                ...
                if hasattr(self, "reasoning_engine") and self.reasoning_engine:
                    verification = self.reasoning_engine.verify(
                        plugin_result,
                        goal_intent=intent,
                        goal_description=goal_res.goal.description
                        if (goal_res and goal_res.goal)
                        else None,
                    )
                    self._think(
                        f"Verifier ΓåÆ verified={verification.verified} goal_fulfilled={verification.goal_fulfilled}"
                    )
                    if not verification.verified and verification.suggested_follow_up:
                        response = (
                            f"{response}\n\n{verification.suggested_follow_up}"
                            if response
                            else verification.suggested_follow_up
                        )
                    ...
                    self.reasoning_engine.record_turn(
                        user_input, intent, entities or {}, response, success
                    )
                    ...
                    try:
                        for _np in getattr(self.plugins, "plugins", None) or []:
                            if hasattr(_np, "record_conversation_turn"):
                                ...
                                if response:
                                    _np.record_conversation_turn(
                                        "assistant", str(response)[:400]
                                    )
                                break
                    except Exception:
                        pass
                    ...
                    if (
                        ...
                        if message_code == "weather:forecast":
                            self._think(
                                f"Storing weather forecast entity with {len(weather_data.get('forecast', []))} days"
                            )
                            self.reasoning_engine.add_entity(
                                WorldEntity(
                                    id="weather_forecast",
                            ...
                            self._think("Weather forecast entity stored successfully")
                            ...
                            _mentioned_tr = next(
                                (k for k in _tr_kw if k in _q_lo), None
                            )
                            if _mentioned_tr:
                                from ai.models.simple_formatters import WeatherFormatter
                                ...
                                _tr_response = WeatherFormatter.format(
                                    weather_data,
                                    entities={"TIME_RANGE": [_mentioned_tr]},
                                )
                                if _tr_response:
                                    self._think(
                                        f"Filtered forecast for '{_mentioned_tr}' ΓÇö replacing full 7-day table"
                                    )
                            ...
                            self._think(
                                f"Stored weather data: {weather_data.get('temperature')}┬░C, {weather_data.get('condition')}"
                            )
                    ...
                    for note in plugin_result.get("notes") or []:
                        n = (
                            ...
                            if isinstance(note, dict)
                                ...
                                and callable(getattr(note, "to_dict"))
                                else {}
                            )
                        )
                ...
                if success:
                    ...
                    if getattr(self, "error_recovery", None):
                        recovery_msg, suggestion = (
                            self.error_recovery.recover_from_error(
                ...
                if getattr(self, "training_collector", None):
                    ...
                    if getattr(self, "learning_engine", None):
                        ...
                        self._think(
                            f"Training data collected (plugin, quality: {quality_score:.2f})"
                        )
            ...
            self._think("No plugin match ΓåÆ using LLM with context")
            ...
            if self.relationship_tracker:
                relationship_response = self._handle_relationship_query(
            ...
            # 3. Alice checks her learned patterns first ΓÇö only use Ollama if she can't answer
            if self.phrasing_learner:
                _conv_thought = {
                    "type": intent,
                ...
                # pass None here ΓÇö _select_tone handles None gracefully.
                _tone_for_check = (
                    self._select_tone(intent, None, user_input)
                    if hasattr(self, "_select_tone")
                    else "helpful"
                ...
                if self.phrasing_learner.can_phrase_myself(
                    _conv_thought, _tone_for_check
                ):
                    _learned_response = self.phrasing_learner.phrase_myself(
                        _conv_thought, tone=_tone_for_check
                    )
                    if _learned_response:
                        self._think(
                            f"Alice answered from learned patterns ΓÇö Ollama not needed for '{intent}'"
                        )
                        if getattr(self, "response_optimizer", None):
            ...
            # 4. Use Gateway for complex knowledge/reasoning (last resort ΓÇö Alice learns from this)
            ...
            enhanced_context = self._build_llm_context(
                user_input_processed, intent, entities, goal_res
            )
            _hidden_summary_for_llm = str(
                (getattr(self, "_internal_reasoning_state", {}) or {}).get(
                    "hidden_situation_summary"
                )
                or ""
            ).strip()
            ...
            if enhanced_context:
                ...
                self.llm.conversation_history.insert(
                    0, {"role": "system", "content": f"Context: {enhanced_context}"}
                )
            ...
            if (
                goal_res
                and goal_res.goal
                and self._should_attach_goal_context(user_input_processed, intent)
            ):
            ...
            try:
                ...
                llm_generation_success = bool(
                    llm_response.success and llm_response.response
                )
                ...
                if llm_response.success and llm_response.response:
                    ...
                    if self.phrasing_learner:
                        ...
                        if self.phrasing_learner.can_phrase_myself(
                            thought_for_check, "helpful"
                        ):
                            self._think(
                                f"Alice learned '{intent}' - can now respond independently!"
                            )
                        else:
                            ...
                            if (
                                hasattr(self.phrasing_learner, "learned_patterns")
                                and pattern in self.phrasing_learner.learned_patterns
                            ):
                                learned_count = len(
                                    self.phrasing_learner.learned_patterns[pattern]
                                )
                            self._think(
                                f"Alice learned from this interaction (confidence in '{intent}' topics growing)"
                            )
                else:
            ...
            if getattr(self, "response_optimizer", None):
                response = self.response_optimizer.optimize(
                    ...
                    {
                        "entities": entities,
                        "goal": goal_res.goal.description
                        if (goal_res and goal_res.goal)
                        else None,
                    },
                )
            ...
            if len(self.llm.conversation_history) > self.llm.config.max_history:
                ...
                if (
                    ...
                    and self.llm.conversation_history[0].get("role") == "system"
                ):
            ...
            self._run_executive_reflection(
                ...
                route=_surface_route,
                prior_confidence=float(
                    intent_confidence if "intent_confidence" in locals() else 0.0
                ),
            )
            ...
            if getattr(self, "training_collector", None):
                ...
                if getattr(self, "learning_engine", None):
                    ...
                    self._think(
                        f"Training data collected (quality: {quality_score:.2f})"
                    )
            ...
            if self._is_error_response(response, expected_domain):
                self._log_error_interaction(
                    ...
                    if any(
                        ...
                        for marker in ["i apologize", "error", "sorry"]
                    )
                    else "wrong_domain",
                    actual_route="TOOL"
            ...
            # Log action for pattern learning
            action = (
                f"{intent}:{entities.get('topic', entities.get('query', 'general'))}"
            )
            self._log_action_for_learning(action)
            ...
            try:
                _learn_decision = str(
                    (getattr(self, "_internal_reasoning_state", {}) or {}).get(
                        "learning_decision", "store"
                    )
                )
                if _learn_decision != "store":
                    self._think(
                        f"Executive learning authority ΓåÆ {_learn_decision} (skip long-term learning)"
                    )
                else:
                    ...
                    if isinstance(learning_result, dict) and not learning_result.get(
                        "stored", True
                    ):
                        reasons = learning_result.get("validation", {}).get(
                            "reasons", []
                        )
                        self._think(
            ...
            route_taken, interaction_success, recovered_via_fallback = (
                self._resolve_turn_success_and_route(
            ...
            self._internal_reasoning_state["turn_completion"] = {
                ...
                "llm_generation_success": bool(llm_generation_success)
                if llm_generation_success is not None
                else None,
            ...
            if hasattr(self, "async_evaluator") and self.async_evaluator:
                ...
                try:
                    alice_confidence = float(
                        intent_confidence if "intent_confidence" in locals() else 0.5
                    )
            ...
            if hasattr(self, "usage_analytics"):
                try:
                    ...
                    llm_used = bool(
                        llm_request_attempted
                        or "ollama_phrased_response" in locals()
                        or "llm_response" in locals()
                    )
                    cached = (
                        intent in cacheable_intents
                        and self._cache_get(user_input, intent) is not None
                    )
                    extra_metadata = None
                    if "plugin_result" in locals() and isinstance(plugin_result, dict):
                        pdata = (
                        ...
                        diagnostics = (
                            pdata.get("diagnostics", {})
                            if isinstance(pdata.get("diagnostics", {}), dict)
                            else {}
                        )
                        extra_metadata = {
            ...
            # ΓöÇΓöÇ Outcome feedback: close the NLP learning loops ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            ...
            try:
                _outcome_success = (
                    ...
                    if "interaction_success" in locals()
                        ...
                        "plugin_result" in locals()
                        and (plugin_result or {}).get("success", True)
                    )
                )
                ...
                if (
                    getattr(self, "knob_bandit", None) is not None
                    and _knob_key is not None
                ):
                    _knob_reward = 1.0 if _outcome_success else -0.5
                    self.knob_bandit.record_outcome(_knob_key, _knob_reward)
                    self._knob_context_key = (
                        None  # reset; propose() will set it next turn
                    )
            except Exception as _outcome_err:
                logger.debug(
                    "[OutcomeFeedback] record_outcome skipped: %s", _outcome_err
                )

            # Memory Management - periodic monitoring and pruning
            if hasattr(self, "memory_growth_monitor") and hasattr(
                self, "memory_pruner"
            ):
                try:
                    ...
                    if self.memory_pruner.should_run():
                        ...
                        if prune_result.get("pruned_count", 0) > 0:
                            logger.info(
                                f"[MemoryPruner] Pruned {prune_result['pruned_count']} old memories"
                            )
                except Exception as e:
            ...
            if (
                ...
                try:
                    ...
                    self.structured_logger.debug(
                        "Response cached", cache_key=cache_key[:50]
                    )
            ...
            if interaction_success:
                self.structured_logger.info(
                    "Request completed with fallback recovery"
            ...
            if self.foundations:
                self._last_interaction = {"input": user_input, "response": response}
                ...
                if self.adaptive_controller is not None:
                    ...
                    if self.adaptive_controller.is_degraded():
                        if not getattr(self, "_safe_mode", False):
                            ...
                            logger.warning(
                                "[AdaptiveController] Degradation detected ΓåÆ safe mode ON"
                            )
                    elif getattr(self, "_safe_mode", False):
                        self._safe_mode = False
                        logger.info(
                            "[AdaptiveController] Recovery detected ΓåÆ safe mode OFF"
                        )
            except Exception:
                pass
    ...
    def _store_interaction(
        ...
        try:
            ...
            # Track topics from intent and entities ΓÇö always move the intent
            ...
            if not self.privacy_mode and getattr(
                ...
                self.memory.store_memory(
                    ...
                    context={
                        ...
                        "turn_contract_route": str(
                            _turn_contract.get("chosen_route") or ""
                        ),
                        "turn_contract_next_action": str(
                            _turn_contract.get("next_action") or ""
                        ),
    ...
    def _handle_command(self, command: str):
        ...
        if cmd == "/help":
            ...
            if readiness_snapshot:
                print("\n Agentic Readiness:")
                print(
                    f"   Last Snapshot: {readiness_snapshot.get('generated_at', 'n/a')}"
                )
            ...
            agentic_loop_report = dict(
                getattr(self, "_last_agentic_cycle_report", {}) or {}
            )
                ...
                try:
                    ...
                    if stats["most_connected_entities"]:
                        ...
                        for entity, count in stats["most_connected_entities"]:
                            print(f"   ΓÇó {entity.title()}: {count} connections")
                ...
                try:
                    ...
                    if stats["relationship_types"]:
                        ...
                        for rel_type, count in stats["relationship_types"].items():
                            print(
                                f"   ΓÇó {rel_type.replace('_', ' ').title()}: {count}"
                            )
                    ...
                    if stats["recent_relationships"]:
                        ...
                        for rel in stats["recent_relationships"][:5]:
                            ...
                            print(
                                f"   ΓÇó {source} {rel_type} {target} (confidence: {confidence:.2f})"
                            )
            ...
            if not memories:
                ...
                for i, (mem_type, mem) in enumerate(memories[:20], 1):  # Show max 20
                    ...
                    importance = "Γÿà" * int(mem.importance * 5)  # Convert to 0-5 stars
            ...
            if not results:
                ...
                for i, result in enumerate(results, 1):
                    ...
                    importance = "Γÿà" * int(result["importance"] * 5)
            ...
            if match:
                ...
                for rel_type, connections in by_type.items():
                    if len(connections) == 1:
                        response_parts.append(f"ΓÇó {connections[0]}")
                    else:
                        response_parts.append(
                            f"ΓÇó {rel_type.replace('_', ' ')}: {', '.join([c.replace(rel_type.replace('_', ' '), '').strip() for c in connections])}"
                        )
                ...
                if second_entity:
                    ...
                    if specific_rels:
                        ...
                        for rel in specific_rels:
                            if rel.source_entity == entity_name.lower():
                                response_parts.append(
                                    f"ΓÇó {entity_name.title()} {rel.relationship_type.replace('_', ' ')} {rel.target_entity.title()}"
                                )
                            else:
                                response_parts.append(
                                    f"ΓÇó {rel.source_entity.title()} {rel.relationship_type.replace('_', ' ')} {entity_name.title()}"
                                )
        ...
        if any(keyword in query_lower for keyword in relationship_info_keywords):
            ...
            if stats["most_connected_entities"]:
                ...
                for entity, count in stats["most_connected_entities"][:3]:
                    response_parts.append(f"ΓÇó {entity.title()}: {count} connections")
        ...
        try:
            ...
            if stats["correction_types"]:
                ...
                for corr_type, count in stats["correction_types"].items():
                    print(f"   ΓÇó {corr_type.replace('_', ' ').title()}: {count}")

            suggestions = self.learning_manager.suggest_improvements()
            if suggestions:
                print("\n Improvement suggestions:")
                for suggestion in suggestions:
                    print(f"   ΓÇó {suggestion}")
