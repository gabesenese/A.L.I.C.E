...
class ContinuousImprovementPipeline:
    ...
    def _parse_test_output_console(self, console_output: str) -> Dict[str, Any]:
        ...
        for line in lines:
            if "Results:" in line and "passed" in line and "failed" in line:
                ...
                if match:
                    ...
                    if self.verbose:
                        print(
                            f"  Found summary: {summary_passed} passed, {summary_failed} failed ({summary_pass_rate}%)"
                        )
                    break
        ...
        if summary_passed is not None and summary_failed is not None:
            ...
            if parsed_passed == summary_passed and parsed_failed == summary_failed:
                ...
                print(
                    f"  ⚠ Parsed {parsed_passed}p/{parsed_failed}f but summary says {summary_passed}p/{summary_failed}f - using summary"
                )
                ...
                while len(failed_ids) < summary_failed:
                    failed_ids.append(
                        {
                            "scenario_id": f"test_failed_{len(failed_ids)}",
                            "error_type": "unknown",
                        }
                    )
    ...
    def _create_retest_file(self, scenario_ids: List[str], output_file: Path):
        ...
        if scenarios_file.exists():
            ...
            print("  Creating basic retest structure (tests may not run)")
    ...
    def apply_automated_fixes(self, analyses: List[FailureAnalysis]) -> Dict[str, Any]:
        ...
        return {"fixes_applied": fixes_applied, "manual_fixes_needed": manual_fixes}
    ...
    def generate_training_from_corrections(
        self, analyses: List[FailureAnalysis], fixes_applied: List[str]
    ...
    def cleanup_mistakes(self, keep_corrected: bool = True) -> Dict[str, Any]:
        ...
        mistake_patterns = [
            ...
            "test_run_iter*.txt",  # Per-iteration run dumps (parsed into memory)
            "test_failures*.txt",
            "test_failures*.md",
        ...
        # Also sweep root workspace for stale *_output.txt and *_results*.txt
        # files generated by redirected runs (e.g. improve_output.txt).
        root_sweep_patterns = [
            "*_output.txt",
            "*_results*.txt",
        ]
        for pattern in root_sweep_patterns:
            for file in self.workspace_root.glob(pattern):
                try:
                    file.unlink()
                    deleted_files.append(str(file))
                    print(f"✗ Deleted stale run artifact: {file.name}")
                except Exception as e:
                    print(f"⚠ Could not delete {file.name}: {e}")
        ...
        return {"deleted": deleted_files, "kept": kept_files}
    ...
    def _save_iteration_history(self, data: Dict[str, Any], status: str):
        ...
        record = {**data, "status": status, "timestamp": datetime.now().isoformat()}
