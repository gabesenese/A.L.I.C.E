...
Last Updated: 2026-04-19 (Canonical launch command + transcript eval pack command)

---

## 🚀 Quick Start

Canonical launch command: `python app/main.py`
...
# Run specific suite (nlp, entity, coreference, conversation, multi-turn, edge-cases)
python test_scenarios.py --suite nlp
python test_scenarios.py --suite notes

# Generate detailed report
python test_scenarios.py --report --output test_report.txt

# Use custom scenarios file
python test_scenarios.py --scenarios custom_scenarios.json
```

### Core Benchmark Gate (Locked 100 Prompts)
```bash
# Build locked core benchmark from real training prompts
python -m tools.auditing.core_benchmark_gate build --training-data data/training/training_data.jsonl --output data/benchmarks/core_100_scenarios.json --size 100 --min-quality 0.7

# Run benchmark and write scorecard
python -m tools.auditing.core_benchmark_gate run --benchmark data/benchmarks/core_100_scenarios.json --scorecard data/benchmarks/scorecard_latest.json

# Gate current scorecard against baseline (fails if objective regresses or is neutral without lower latency)
python -m tools.auditing.core_benchmark_gate gate --scorecard data/benchmarks/scorecard_latest.json --baseline data/benchmarks/scorecard_baseline.json

# First-run baseline bootstrap (only run once, or when intentionally resetting baseline)
python -m tools.auditing.core_benchmark_gate gate --scorecard data/benchmarks/scorecard_latest.json --baseline data/benchmarks/scorecard_baseline.json --write-baseline

# Run benchmark and gate in one command (recommended for CI)
python -m tools.auditing.core_benchmark_gate run-gate --benchmark data/benchmarks/core_100_scenarios.json --scorecard data/benchmarks/scorecard_latest.json --baseline data/benchmarks/scorecard_baseline.json
```

### Transcript Evaluation Pack (Day 2)
```bash
# Build a 50-turn transcript eval pack from interaction logs
python scripts/evals/build_transcript_eval_pack.py --input data/test_learning.jsonl --output tests/e2e/data/transcript_eval_pack_50.jsonl --limit 50
```
...
**Philosophy**: Automatically test → fix → train → verify → cleanup loop.
Delete mistakes, keep only corrected training data.

```bash
# Run full improvement iteration (test, analyze, train, verify, cleanup)
python improve_alice.py

# Run with VERBOSE mode - see real-time test output
python improve_alice.py --verbose
python improve_alice.py -v  # Short form

# Run multiple iterations until 100% pass rate
python improve_alice.py --iterations 5

# Verbose mode with multiple iterations
python improve_alice.py --verbose --iterations 5

# Auto-apply safe fixes without manual review
python improve_alice.py --auto-fix

# Skip training phase (for testing pipeline only)
python improve_alice.py --skip-training
```

**What it does:**
1. **Phase 1**: Run test scenarios
2. **Phase 2**: Analyze failures (categorize by type, identify root causes)
3. **Phase 3**: Apply automated fixes where possible
4. **Phase 4**: Generate corrected training data from failures
5. **Phase 5**: Train A.L.I.C.E on corrections
6. **Phase 6**: Re-run failed tests to verify improvements
7. **Phase 7**: Delete mistake artifacts, keep corrected data

**Files managed:**
- ✓ KEPT: `auto_corrected.jsonl`, `improvement_history.jsonl`, `successful_interactions.jsonl`
- ✗ DELETED: `test_output*.txt`, `failed_*.jsonl`, `debug_*.py`, mistake artifacts
...
### Startup Diagnostics & QA Gate
```bash
# Run Startup Doctor (fast profile, default)
python -m tools.auditing.startup_doctor --root . --profile fast

# Run Startup Doctor with deeper diagnostics
python -m tools.auditing.startup_doctor --root . --profile standard
python -m tools.auditing.startup_doctor --root . --profile deep

# Run Startup Doctor and fail CI on degraded/blocked status
python -m tools.auditing.startup_doctor --root . --profile fast --strict

# Write Startup Doctor report to custom path
python -m tools.auditing.startup_doctor --root . --profile fast --output data/qa/startup_health_summary.json

# Run learning-data QA gate (policy checks + exit code)
python -m tools.auditing.learning_data_qa_gate --root .

# Run QA gate with warning-trend baseline checks
python -m tools.auditing.learning_data_qa_gate --root . --baseline data/qa/warning_baseline.json

# Bootstrap/update warning baseline from current run
python -m tools.auditing.learning_data_qa_gate --root . --baseline data/qa/warning_baseline.json --write-baseline

# Save gate report JSON for CI artifacts
python -m tools.auditing.learning_data_qa_gate --root . --report data/qa/gate_report.json

# Run learned-data QA report and print grouped summaries
python tools/auditing/training_data_auditor.py qa --root . --report data/qa/learning_data_qa_report.json

# Quarantine and clean critical learned-data records
python tools/auditing/training_data_auditor.py qa-clean --root . --min-severity critical --report data/qa/learning_data_qa_cleanup_report.json
```

### Startup Doctor Environment Controls
```bash
# Disable startup diagnostics at ALICE initialization
# PowerShell
$env:ALICE_STARTUP_DOCTOR = "0"

# Set startup diagnostics profile at initialization
# PowerShell
$env:ALICE_STARTUP_PROFILE = "fast"      # or standard, deep

# Run A.L.I.C.E with startup diagnostics enabled
python app/main.py
```
