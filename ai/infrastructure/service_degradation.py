...
class ServiceDegradationHandler:
    ...
    def _initialize_services(self):
        ...
        services = [
            ...
            (
                "documents",
                "Document query unavailable - stored documents not searchable",
            ),
        ]
    ...
    def mark_service_failed(self, service_name: str, error: Exception):
        ...
        if service.failure_count >= self.max_failures_before_disable:
            if service.is_available:
                logger.warning(
                    f" Service '{service_name}' is now UNAVAILABLE after {service.failure_count} failures"
                )
                service.is_available = False
        else:
            logger.warning(
                f"Service '{service_name}' failed ({service.failure_count}/{self.max_failures_before_disable}): {error}"
            )
    ...
    def get_service_status(self, service_name: str) -> ServiceStatus:
        ...
        return self.services.get(
            service_name,
            ServiceStatus(
                service_name=service_name, is_available=True, last_check=datetime.now()
            ),
        )
...
def with_ollama_fallback(
    fallback_response: str = "I'm having trouble generating a response right now.",
):
...
def with_email_fallback(
    fallback_response: str = "Email service is temporarily unavailable.",
):
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            handler = get_degradation_handler()
            return handler.with_fallback(
                "gmail", lambda: func(*args, **kwargs), fallback_value=fallback_response
            )
...
def with_calendar_fallback(
    fallback_response: str = "Calendar service is temporarily unavailable.",
):
