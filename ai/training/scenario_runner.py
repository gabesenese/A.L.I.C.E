...
class ScenarioRunner:
    ...
    def __init__(
        self, nlp_processor=None, router=None, output_dir: str = "data/training"
    ):
        """
    ...
    def run_scenario(
        ...
        for turn_index, turn in enumerate(turns):
            ...
            logger.info(
                f"  Turn {turn_index + 1} {status}: {result.user_input[:50]}..."
            )
            logger.info(
                f"    Intent: {result.detected_intent} (expected: {result.expected_intent})"
            )
            if result.expected_tool:
                logger.info(
                    f"    Tool: {result.used_tool} (expected: {result.expected_tool})"
                )

        return scenario_results, all_success

    def _run_turn(
        self, scenario_name: str, turn_index: int, turn: Dict[str, Any]
    ) -> ScenarioResult:
        """
        ...
        if self.router and detected_intent:
            try:
                route_result = self.router.route(
                    intent=detected_intent, confidence=intent_confidence, entities={}
                )
        ...
        tool_match = (expected_tool is None and used_tool is None) or (
            used_tool == expected_tool
        )
    ...
    def save_training_data(self, output_file: str = None):
        ...
        if output_file is None:
            output_file = os.path.join(
                self.output_dir, "synthetic_from_scenarios.jsonl"
            )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        intent_accuracy = (
            (sum(1 for r in self.results if r.intent_match) / total_turns * 100)
            if total_turns > 0
            else 0
        )
        tool_accuracy = (
            (sum(1 for r in self.results if r.tool_match) / total_turns * 100)
            if total_turns > 0
            else 0
        )
        ...
        return {
            ...
            "success_rate": (
                (successful_turns / total_turns * 100) if total_turns > 0 else 0
            ),
...
def main():
    ...
    print(
        f"Turn success rate: {(stats['successful_turns'] / stats['total_turns'] * 100) if stats['total_turns'] > 0 else 0:.1f}%"
    )
    print()
