...
class ActiveLearningManager:
    ...
    def _save_data(self):
        ...
        try:
            ...
            with open(self.patterns_file, "w", encoding="utf-8") as f:
                json.dump(
                    [asdict(p) for p in self.learning_patterns],
                    f,
                    indent=2,
                    ensure_ascii=False,
                )
    ...
    def _analyze_new_correction(self, correction: Correction):
        ...
        if similar_corrections < self.MIN_EXAMPLES_TO_APPLY - 1:
            print(
                f"Shadow mode: Need {self.MIN_EXAMPLES_TO_APPLY - similar_corrections - 1} more example(s) before creating pattern"
            )
            return
        ...
        for existing in self.corrections:
            if (
                ...
                if (
                    ...
                    if (
                        existing.original_output == correction.original_output
                        and existing.corrected_output == correction.corrected_output
                    ):
                        count += 1
        return count
    ...
    def should_apply_pattern(self, pattern: LearningPattern) -> bool:
        ...
        if pattern.validation_count < self.MIN_EXAMPLES_TO_APPLY:
            print(
                f" Pattern {pattern.pattern_id} needs more examples ({pattern.validation_count}/{self.MIN_EXAMPLES_TO_APPLY})"
            )
            return False
        ...
        if pattern.confidence < self.MIN_CONFIDENCE_TO_APPLY:
            print(
                f" Pattern {pattern.pattern_id} confidence too low ({pattern.confidence:.2f} < {self.MIN_CONFIDENCE_TO_APPLY})"
            )
            return False
        ...
        if pattern.usage_count > 0 and pattern.success_rate < self.MIN_SUCCESS_RATE:
            print(
                f" Pattern {pattern.pattern_id} success rate too low ({pattern.success_rate:.2f} < {self.MIN_SUCCESS_RATE})"
            )
            return False
    ...
    def apply_learning(
        ...
        # Don't override intent when NLP is already highly confident.
