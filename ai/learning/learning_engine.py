...
class LearningEngine:
    ...
    def log_llm_call(
        ...
        if similar_count >= 3:
            logger.info(
                f"Pattern opportunity: '{user_input[:50]}...' asked {similar_count} times - consider creating pattern"
            )
            return {
...
class AutoCorrectionEngine:
    ...
    def process_scenario_results(self, results: List[Dict]) -> Dict[str, Any]:
        ...
        if corrections_added > 0:
            ...
            logger.info(
                f"[AutoCorrection] Added {corrections_added} corrections ({intent_mismatches} intent, {route_mismatches} route)"
            )
    ...
    def _find_existing_correction(self, correction: Dict[str, Any]) -> Optional[Dict]:
        ...
        for existing in self.corrections:
            if (
                existing.get("user_input") == correction.get("user_input")
                and existing.get("expected_intent") == correction.get("expected_intent")
                and existing.get("expected_route") == correction.get("expected_route")
            ):
                return existing
        return None
...
def get_auto_correction_engine(
...
def get_pattern_promotion_engine(
    project_root: Optional[Path] = None,
