        ...
        *,
        ...
        if "analyze" in text and any(
            ...
            return {
                "intent": cur_intent,
                "confidence": conf,
                "reason": "finance_context_refine",
            }

        # Generic short follow-up inherits last actionable domain lightly.
        if (
            len(text.split()) <= 6
            and cur_intent.startswith("conversation:")
            and ":" in last
            and not last.startswith("conversation:")
        ):
            cur_intent = last
            conf = max(conf, 0.67)
            return {
                "intent": cur_intent,
                "confidence": conf,
                "reason": "short_followup_domain_inherit",
            }
