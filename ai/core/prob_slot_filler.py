...
_TITLE_PATS: List[re.Pattern] = [
    re.compile(
        r'(?:called|titled?|named?)\s+["\']?(.+?)["\']?(?:\s+tagged|\s+with tag|\s+and\s|\s*$)',
        re.I,
    ),
    re.compile(r'"([^"]{2,80})"'),
    re.compile(r"'([^']{2,80})'"),
    re.compile(
        r"(?:create|make|write|save|add)\s+(?:a\s+)?(?:note\s+)?(?:about\s+)?([A-Z][^\s,]{1,40})",
        re.I,
    ),
]
...
_QUERY_PATS: List[re.Pattern] = [
    re.compile(
        r"(?:find|search|look\s+for)\s+(?:my\s+)?(?:notes?\s+(?:about|on|for)\s+)?(.+?)(?:\s+tagged|\s+with\s+tag|\s*$)",
        re.I,
    ),
    re.compile(
...
_EVENT_TITLE_PATS: List[re.Pattern] = [
    re.compile(
        r"(?:schedule|add|create|set|book)\s+(?:a\s+)?(?:meeting|event|appointment|reminder\s+for)?\s*(.+?)\s+(?:at|on|for|tomorrow|today|\d)",
        re.I,
    ),
    re.compile(
        r"(?:meeting|event|appointment)\s+(?:called|titled?|named?|about)\s+(.+?)(?:\s+at|\s+on|\s*$)",
        re.I,
    ),
]
...
_RECIPIENT_PATS: List[re.Pattern] = [
    ...
    re.compile(
        r"(?:send|write|email)\s+(?:an?\s+(?:email|message)\s+)?to\s+(.+?)(?:\s+about|\s+saying|\s+with|\s*$)",
        re.I,
    ),
]
...
_DATE_PATS: List[re.Pattern] = [
    re.compile(
        r"\b(today|tomorrow|yesterday|next\s+\w+|this\s+\w+|\d{1,2}/\d{1,2}(?:/\d{2,4})?|\w+\s+\d{1,2}(?:st|nd|rd|th)?)\b",
        re.I,
    ),
]
