...
class ResponseVarianceEngine:
    ...
    def __init__(self, llm_generator=None, phrasing_learner=None):
        ...
        self.successful_patterns = defaultdict(lambda: {
            'avg_length': 0,
            'uses_examples': 0,
            'uses_confirmation': 0,
            'directness_scores': [],
    ...
    def _detect_user_mood(self, conversation_history: List[Dict]) -> str:
        ...
        for turn in recent_turns:
            ...
            if user_input.count('?') > 1:
                confusion_signals += 1
            if any(phrase in user_input for phrase in ['what do you mean', "don't understand", 'confused', 'huh']):
                confusion_signals += 2
    ...
    def _get_recent_similar_responses(self, user_id: str, intent_type: str, n: int = 3) -> List[str]:
        ...
        similar = [
            resp for resp_intent, resp in history
            if resp_intent == intent_type
        ]
    ...
    def _generate_with_variance(self, context: ResponseContext, constraints: Dict) -> str:
        ...
        # Core instruction
        prompt_parts.append(f"Generate a natural, conversational response for this situation:")
