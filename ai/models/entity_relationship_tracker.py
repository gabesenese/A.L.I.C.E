...
class EntityRelationshipTracker:
    ...
    def __init__(self, data_dir: str = "data"):
        ...
        logger.info(
            f"Entity Relationship Tracker initialized with {len(self.entities)} entities and {len(self.relationships)} relationships"
        )
    ...
    def _build_relationship_patterns(self) -> Dict[str, List[Tuple[str, float]]]:
        ...
        return {
            ...
            RelationshipType.PARENT: [
                (
                    r"(\w+)(?:\s+is)?\s+(?:my|his|her)\s+(?:father|dad|mother|mom|parent)",
                    0.9,
                ),
                (
                    r"(\w+)(?:\s+and\s+\w+)?\s+(?:are|is)\s+(?:my|his|her)\s+parents?",
                    0.9,
                ),
            ],
            ...
            RelationshipType.WORKS_AT: [
                ...
                (
                    r"(\w+)(?:\s+is)?\s+(?:an?\s+)?(?:employee|worker)\s+at\s+(\w+(?:\s+\w+)*)",
                    0.8,
                ),
            ...
            RelationshipType.LIVES_IN: [
                ...
                (
                    r"(\w+)(?:'s)?\s+(?:home|house)\s+(?:is\s+)?in\s+(\w+(?:\s+\w+)*)",
                    0.8,
                ),
            ],
            ...
            RelationshipType.STUDIED_AT: [
                ...
                (
                    r"(\w+)(?:\s+is)?\s+(?:a\s+)?(?:student|graduate)\s+(?:of\s+)?(?:at\s+)?(\w+(?:\s+\w+)*)",
                    0.7,
                ),
            ],
        }
    ...
    def _find_relationship(
        ...
        for rel in self.relationships:
            if (
                rel.source_entity == source_lower
                and rel.target_entity == target_lower
    ...
    def get_entity_network(
        ...
        entity_lower = entity_name.lower()
        visited = set()
        network = {"center": entity_name, "entities": {}, "relationships": []}
    ...
    def query_relationships(self, query: str) -> List[EntityRelationship]:
        ...
        for rel in self.relationships:
            if (
                query_lower in rel.source_entity.lower()
                or query_lower in rel.target_entity.lower()
                or query_lower in rel.relationship_type.lower()
                or query_lower in rel.context.lower()
            ):
                results.append(rel)
