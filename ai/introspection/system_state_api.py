...
class SystemStateAPI:
    ...
    def get_processor_state(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "reasoning_engine": "active" if hasattr(self.alice, 'reasoning') and self.alice.reasoning else "inactive",
            "learning_engine": "active" if hasattr(self.alice, 'learning') and self.alice.learning else "inactive",
        }
        ...
        if mem_sys:
            try:
                ...
                if hasattr(mem_sys, 'episodic_memory'):
                    state["episodic_memories"] = len(mem_sys.episodic_memory) if hasattr(mem_sys.episodic_memory, '__len__') else 0
                
                if hasattr(mem_sys, 'semantic_memory'):
                    state["semantic_memories"] = len(mem_sys.semantic_memory) if hasattr(mem_sys.semantic_memory, '__len__') else 0
    ...
    def format_system_report(self) -> str:
        ...
        lines.append(f"Session: {session.get('session_id')} | Turns: {session.get('turn_count')} | Uptime: {session.get('uptime_seconds')}s")
        lines.append("")
        ...
        # Active state
        memory = snapshot.get("memory", {})
        lines.append(f"Memory: {memory.get('episodic_memories', 0)} episodic, {memory.get('semantic_memories', 0)} semantic ({memory.get('memory_usage_mb', 0):.0f}MB)")
