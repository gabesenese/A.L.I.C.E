        ...
        try:
            if operation == "create":
                ...
                if not destination:
                    return TaskResult(
                        success=False,
                        message="Destination required for copy",
                ...
                if not destination:
                    return TaskResult(
                        success=False,
                        message="Destination required for move",
                        error="Missing destination",
                    )
    ...
    def create_workflow(self, workflow_name: str, tasks: List[Dict]) -> TaskResult:
        ...
        for i, task in enumerate(tasks, 1):
            ...
            try:
                ...
                results.append({"step": i, "task": task, "result": result.to_dict()})
                ...
                results.append({"step": i, "task": task, "error": str(e)})
                break
