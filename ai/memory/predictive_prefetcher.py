...
class PredictivePrefetcher:
    ...
    def predict_next_actions(
        self, current_intent: str, entities: Dict
    ) -> List[Prediction]:
        ...
        if current_intent in self.sequential_patterns:
            ...
            if total > 0:
                for action, count in sorted(
                    next_actions.items(), key=lambda x: x[1], reverse=True
                )[:3]:
    ...
    def prefetch_for_prediction(self, prediction: Prediction):
        ...
        logger.debug(
            f"[Prefetcher] Prefetched for: {prediction.action} (confidence: {prediction.confidence:.2f})"
        )
