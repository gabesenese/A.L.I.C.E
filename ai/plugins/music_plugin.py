    ...
    def setup_spotify(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                print(
                    f"Spotify connected for user: {user.get('display_name', 'Unknown')}"
                )
                return True
            except Exception as e:
    ...
    def _scan_directory(self, directory: str) -> None:
        ...
        try:
            for root, dirs, files in os.walk(directory):
                for file in files:
                    if any(
                        file.lower().endswith(ext) for ext in self.supported_formats
                    ):
            ...
            if METADATA_AVAILABLE:
                try:
                    if file_path.lower().endswith(".mp3"):
                        ...
                        title = (
                            str(audio_file.get("TIT2", [title])[0])
                            if audio_file.get("TIT2")
                            else title
                        )
                        artist = (
                            str(audio_file.get("TPE1", ["Unknown Artist"])[0])
                            if audio_file.get("TPE1")
                            else "Unknown Artist"
                        )
                        album = (
                            str(audio_file.get("TALB", ["Unknown Album"])[0])
                        ...
                        title = (
                            audio_file.get("\xa9nam", [title])[0]
                            if audio_file.get("\xa9nam")
                            else title
                        )
                        artist = (
                            audio_file.get("\xa9ART", ["Unknown Artist"])[0]
                            if audio_file.get("\xa9ART")
                            else "Unknown Artist"
                        )
                        album = (
                            audio_file.get("\xa9alb", ["Unknown Album"])[0]
                            if audio_file.get("\xa9alb")
                            else "Unknown Album"
                        )
                        duration = (
                            int(audio_file.info.length)
    ...
    def play_file(self, file_path: str) -> bool:
        ...
        try:
            ...
            self.current_song = next(
                (song for song in self.music_library if song.uri == file_path), None
            )
...
class MusicPlugin(PluginInterface):
    ...
    def prompt_local_music_selection(self) -> str:
        ...
        mp3_files = [
            song
            for song in self.local_manager.music_library
            if song.uri.lower().endswith(".mp3")
        ]
        if not mp3_files:
        ...
        for i, song in enumerate(mp3_files, 1):
            response += (
                f"{i}. {song.title} by {song.artist} ({os.path.basename(song.uri)})\n"
            )
    ...
    def __init__(self):
        ...
        self.pending_local_selection = (
            None  # Initialize pending selection for local music
        )
    ...
    def can_handle(self, intent: str, entities: Dict[str, Any]) -> bool:
        ...
        if (
            music_entities.get("action")
            or music_entities.get("song")
            or music_entities.get("artist")
        ):
            return True
    ...
    def execute(
        self, intent: str, query: str, entities: Dict[str, Any], context: Dict
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if intent in ["MUSIC_PLAY", "PLAY"] or action == "play":
                return self._handle_play_request(
                    song, artist, album, playlist, user_input
                )
    ...
    def _handle_play_request(
        self, song: str, artist: str, album: str, playlist: str, user_input: str
    ) -> str:
    ...
    def _handle_search_request(
        self, song: str, artist: str, album: str, user_input: str
    ) -> str:
    ...
    def _play_by_search(self, query: str) -> str:
        ...
        prefer_youtube = any(
            platform in query.lower() for platform in ["youtube", "yt"]
        )
        prefer_spotify = any(platform in query.lower() for platform in ["spotify"])

        # Try YouTube Music first (most reliable, no API needed)
        if self.youtube_controller.is_available and (
            prefer_youtube or self.current_source in ["youtube", "auto"]
        ):
            if song and artist:
        ...
        if (
            not prefer_youtube
            and self.desktop_spotify.is_available
            and self.desktop_spotify.is_spotify_running()
        ):
        ...
        if (
            not prefer_youtube
            and self.spotify_manager.is_authenticated
            and self.current_source in ["spotify", "auto"]
        ):
        ...
        # Try local files
        if self.local_manager.is_initialized and self.current_source in [
            "local",
            "auto",
        ]:
    ...
    def _extract_volume_from_input(self, user_input: str) -> Optional[int]:
        ...
        patterns = [r"(\d{1,3})%", r"volume (\d{1,3})", r"to (\d{1,3})", r"(\d{1,3})"]
