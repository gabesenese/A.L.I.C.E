            ...
            try:
                ...
                if semantic_result and semantic_result["confidence"] > 0.5:
                    ...
                    if plugin and plugin.enabled:
                        ...
                        if not can_handle:
                            logger.info(
                                f" Semantic match {plugin_name} rejected by can_handle - trying traditional matching"
                            )
                            # Fall through to traditional matching
                        else:
                            logger.info(
                                f" Semantic match: {plugin_name}:{semantic_result['action']} (confidence: {semantic_result['confidence']:.2f})"
                            )
                            try:
    ...
    def execute(self, intent: str, query: str, entities: Dict, context: Dict) -> Dict:
        try:
            ...
            return {
                "success": False,
                "response": None,
                "data": {"error": str(e), "message_code": "weather:error"},
            }
...
class SystemControlPlugin(PluginInterface):
    ...
    def execute(self, intent: str, query: str, entities: Dict, context: Dict) -> Dict:
        ...
        return {
            "success": True,
            "response": None,
            "data": {"action": action, "message_code": message_code},
        }
...
class WebSearchPlugin(PluginInterface):
    ...
    def execute(self, intent: str, query: str, entities: Dict, context: Dict) -> Dict:
        ...
        return {
            ...
            "data": {"search_query": search_terms, "action": "open_browser"},
        }
