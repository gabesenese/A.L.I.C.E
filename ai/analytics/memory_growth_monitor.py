...
class MemoryGrowthMonitor:
    ...
    def take_snapshot(self, memory_system) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"[MemoryMonitor] Snapshot taken: {snapshot['file_stats']['total_size_mb']}MB, {snapshot['memory_stats']['total_memories']} memories"
            )
    ...
    def generate_text_visualization(self, days: int = 30) -> str:
        ...
        if not growth.get("insufficient_data"):
            ...
            lines.append("-" * 80)
            lines.append(
                f"  Size Growth: {growth['size_growth_mb']}MB ({growth['size_growth_rate_mb_per_day']:.2f}MB/day)"
            )
            lines.append(
                f"  Memory Growth: {growth['memory_growth']} memories ({growth['memory_growth_rate_per_day']:.2f}/day)"
            )
            lines.append("")
