...
def test_weighted_memory_ranking_prefers_recent_confident_reliable(
    ...
    base_results = [
        {
            "id": "a",
            "content": entry_map["a"].content,
            "similarity": 0.93,
            "importance": 0.45,
            "timestamp": entry_map["a"].timestamp,
            "type": "episodic",
            "access_count": 1,
            "tags": [],
        },
        {
            "id": "b",
            "content": entry_map["b"].content,
            "similarity": 0.86,
            "importance": 0.9,
            "timestamp": entry_map["b"].timestamp,
            "type": "procedural",
            "access_count": 1,
            "tags": [],
        },
        {
            "id": "c",
            "content": entry_map["c"].content,
            "similarity": 0.84,
            "importance": 0.7,
            "timestamp": entry_map["c"].timestamp,
            "type": "semantic",
            "access_count": 1,
            "tags": [],
        },
    ...
    monkeypatch.setattr(
        mem, "recall_memory", lambda query, top_k=5, min_similarity=0.35: base_results
    )
    monkeypatch.setattr(
        mem, "_get_memory_by_id", lambda memory_id: entry_map[memory_id]
    )
...
def test_get_context_for_llm_uses_weighted_top_memories(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(
        mem, "recall_memory_weighted", lambda query, top_k=3: weighted[:top_k]
    )
