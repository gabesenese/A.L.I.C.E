...
EXACT_PROMPT = (
    "i want to learn the foundations an advanced assistant system should have in today's world with no fiction"
)
...
def test_answerability_gate_detects_specific_domain_questions_and_ignores_ambiguous_phrasing():
    ...
    assert alice._is_answerability_direct_question("how does optimizer training work in nlp models?") is True
    assert (
    ...
    assert alice._is_answerability_direct_question("how does optimizer stuff work?") is False
...
def test_answerability_gate_fallback_handles_existing_framework_inventory_prompt():
    ...
    response = alice._answerability_gate_fallback_response("explain to me each existing framework")
...
def test_fast_lane_sanitizer_removes_clarification_dead_end_for_answerable_question():
    ...
    response = alice._sanitize_fast_lane_response(
        response=(
            "Please clarify what you mean. In NLP model training, the optimizer updates weights using gradient steps."
        ),
...
def test_agent_algorithm_question_detector_matches_user_style_query():
    ...
    assert alice._is_agent_algorithm_question("and which algorithms are best for an ai agent") is True
...
def test_action_cue_detector_ignores_conceptual_build_prompt():
    ...
    assert (
        alice._has_explicit_action_cue("how can i create an ai like assistant with todays technology and no fiction")
        is False
    )
...
def test_llm_context_injects_recent_weather_only_for_weather_relevant_queries() -> None:
    ...
    alice.self_reflection = SimpleNamespace(get_codebase_summary=lambda: {"base_path": ".", "total_files": 0})
