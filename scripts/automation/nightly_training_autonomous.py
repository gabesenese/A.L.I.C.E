...
def run_autonomous_nightly_training():
    ...
    # Track overall stats
    pipeline_stats = {"start_time": datetime.now(), "phases_completed": []}
    ...
    try:
        ...
        try:
            from ai.training.scenario_generator import (
                generate_scenarios,
                generate_scenarios_from_errors,
            )
        ...
        runner = ScenarioRunner(llm_policy="minimal", use_teacher=True)
        ...
        logger.info("[PHASE 3] Threshold Adjustments:")
        ...
        from ai.models.ml_models import get_router_classifier, get_intent_refiner
        ...
        report = {
            ...
            "pipeline_summary": {
                ...
                "scenario_success_rate": f"{success_count / scenario_count * 100:.1f}%"
                if scenario_count > 0
                else "N/A",
            },
            "teacher_comparison": {
